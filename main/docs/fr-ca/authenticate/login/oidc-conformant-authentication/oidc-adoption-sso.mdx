...
Dans le contexte du pipeline d’authentification conforme à OIDC, l’authentification unique (<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">SSO</Tooltip>) doit se faire au niveau du serveur d’autorisation (c.-à-d. Auth0) plutôt qu’à celui de l’application, ce qui signifie que vous devez utiliser Connexion universelle et rediriger les utilisateurs vers la page de connexion. Pour en savoir plus, consultez [Connexion universelle](/docs/fr-ca/authenticate/login/auth0-universal-login) et [Authentification unique](/docs/fr-ca/authenticate/single-sign-on).
