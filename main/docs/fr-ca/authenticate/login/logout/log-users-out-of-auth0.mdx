...
Auth0 implémente la [déconnexion initié par RP 1.0](https://openid.net/specs/openid-connect-rpinitiated-1_0.html) d’<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">OpenID</Tooltip> Connect pour la déconnexion de l’utilisateur final. Cette norme fait partie de l’ensemble de [spécifications finales](https://openid.net/developers/specs/) OpenID Connect.
...
Vous pouvez activer **RP-Initiated Logout End Session Endpoint Discovery (Découverte du point de terminaison de fin de session de déconnexion initiée par RP)** dans l’<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">Auth0 Dashboard</Tooltip> ou avec l’Auth0 Management API.
...
2. Appelez le point de terminaison [Mise à jour des paramètres du locataire](/docs/api/management/v2/tenants/patch-settings) de Management API avec la charge utile suivante :
...
La plupart des [bibliothèques de trousses SDK](/docs/libraries) d’Auth0 sont conformes à l’OIDC et prennent en charge la déconnexion initiée par la partie de confiance.
...
<td>Facultatif</td>
<td>Valeur de l’identifiant de session (<code>sid</code>) qui indique l’utilisateur à déconnecter.</td>
</tr>
<tr>
...
<td>Facultatif</td>
<td>Valeur opaque que l’application ajoute à la requête initiale de déconnexion, et qu’Auth0 inclut lors de la redirection vers le <code>post_logout_redirect_uri</code>.</td>
</tr>
<tr>
...
When you call the Management API [Update a client](/docs/api/management/v2/clients/patch-clients-by-id) endpoint, it overwrites all existing configuration data for the fields provided in the request body.

To avoid accidental loss of data, call the Management API [Get a client](/docs/api/management/v2/clients/get-clients-by-id) endpoint first to retrieve your application’s current configuration data.
...
When you call the Management API [Update tenant settings](/docs/api/management/v2/tenants/patch-settings) endpoint, it overwrites all existing configuration data for the fields provided in the request body.

To avoid accidental loss of data, call the Management API [Get tenant settings](/docs/api/management/v2/tenants/tenant-settings-route) endpoint to retrieve your tenant’s current configuration data first.
...
2. Appelez le point de terminaison [Mettre à jour les paramètres du locataire](/docs/api/management/v2/tenants/patch-settings) de Management API et assurez-vous d’inclure l’URL dans la valeur du champ `allowed_logout_urls` dans le corps de la demande.
...
<Frame>![null](//images.ctfassets.net/cdy7uua7fh8z/5Sycg1aMJ1CLZwJj19Omv4/9677dc3b3549f9ebe7dee51aba3029c7/Logout_-_French.png)</Frame>
...
2. Désactivez la bascule **Afficher la confirmation de l’utilisateur final de déconnexion initiée par RP**.
   ...
   <Frame>![null](//images.ctfassets.net/cdy7uua7fh8z/37K4hLjhSBMGvdGP9OEP7W/d3761ebf29cab3aa184361df9a914735/Login_and_Logout_-_French.png)</Frame>
