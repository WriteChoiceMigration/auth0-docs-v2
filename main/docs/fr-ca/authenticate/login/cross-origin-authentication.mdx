...
* Enable a [Custom Domain](/docs/fr-ca/customize/custom-domains) on your tenant and host your web application in a domain that has the same top-level domain as your Auth0 <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-3" tip="">custom domain</Tooltip>. For example, you host an application at `https://northwind.com` and set your Auth0 custom domain as `https://login.northwind.com`. De cette manière, les témoins ne sont plus considérés comme provenant d’une tierce partie (parce que votre locataire Auth0 et votre application utilisent le même domaine de niveau supérieur), et ne sont donc pas bloqués par les navigateurs.
