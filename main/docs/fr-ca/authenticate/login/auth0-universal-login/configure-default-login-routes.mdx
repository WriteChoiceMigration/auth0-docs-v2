...
request.AddParameter("application/json", "{\"initiate_login_uri\": \"\"}", ParameterType.RequestBody);
...
func main() {
url := "https://{yourDomain}/api/v2/clients/{yourClientId}"
payload := strings.NewReader("{\"initiate_login_uri\": \"\"}")
...
```
```java java.java
HttpResponse response = Unirest.patch("https://{yourDomain}/api/v2/clients/{yourClientId}")
.header("content-type", "application/json")
.header("authorization", "Bearer API2_ACCESS_TOKEN")
.header("cache-control", "no-cache")
...
@"authorization": @"Bearer API2_ACCESS_TOKEN",
@"cache-control": @"no-cache" };
NSDictionary \*parameters = @{ @"initiate_login_uri": @"" };
...
CURLOPT_POSTFIELDS => "{\"initiate_login_uri\": \"\"}",
CURLOPT_HTTPHEADER => [
"authorization: Bearer API2_ACCESS_TOKEN",
"cache-control: no-cache",
"content-type: application/json"
...
request.body = "{\"initiate_login_uri\": \"\"}"
...
let parameters = ["initiate_login_uri": ""] as [String : Any]
...
request.AddHeader("authorization", "Bearer API2_ACCESS_TOKEN");
request.AddHeader("cache-control", "no-cache");
request.AddParameter("application/json", "{\"default_redirection_uri\": \"\"}", ParameterType.RequestBody);
IRestResponse response = client.Execute(request);
```
...
func main() {
url := "https://{yourDomain}/api/v2/tenants/settings"
payload := strings.NewReader("{\"default_redirection_uri\": \"\"}")
req, _ := http.NewRequest("PATCH", url, payload)
req.Header.Add("content-type", "application/json")
req.Header.Add("authorization", "Bearer API2_ACCESS_TOKEN")
req.Header.Add("cache-control", "no-cache")
...
@"authorization": @"Bearer API2_ACCESS_TOKEN",
@"cache-control": @"no-cache" };
NSDictionary \*parameters = @{ @"default_redirection_uri": @"" };
NSData \*postData = [NSJSONSerialization dataWithJSONObject:parameters options:0 error:nil];
...
CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
CURLOPT_CUSTOMREQUEST => "PATCH",
CURLOPT_POSTFIELDS => "{\"default_redirection_uri\": \"\"}",
CURLOPT_HTTPHEADER => [
"authorization: Bearer API2_ACCESS_TOKEN",
"cache-control: no-cache",
"content-type: application/json"
...
import http.client
conn = http.client.HTTPSConnection("")
payload = "{\"default_redirection_uri\": \"\"}"
headers = {
'content-type': "application/json",
'authorization': "Bearer API2_ACCESS_TOKEN",
'cache-control': "no-cache"
...
request["authorization"] = 'Bearer API2_ACCESS_TOKEN'
request["cache-control"] = 'no-cache'
request.body = "{\"default_redirection_uri\": \"\"}"
response = http.request(request)
puts response.read_body
```
```swift swift.swift
...
let parameters = ["default_redirection_uri": ""] as [String : Any]
let postData = JSONSerialization.data(withJSONObject: parameters, options: [])
...
Lorsqu’une application lance le processus de connexion, elle navigue vers `https://{yourDomain}/authorize` avec un ensemble de [paramètres requis](/docs/api/authentication#login). Auth0 redirige ensuite les utilisateurs vers une page `https://{yourDomain}/login`, avec une URL qui ressemble à :
...
Pour les locataires utilisant la connexion universelle, le point de terminaison [`/post-password-change`](/docs/api/management/v2/#!/Tickets/post_password_change) permet de rediriger les utilisateurs vers une application en particulier. Lorsque `client_id` est indiqué et que l’URI de connexion de l’application est défini, les utilisateurs verront un bouton les renvoyant à l’application après avoir effectué une réinitialisation de mot de passe.
...
```
```java java.java
HttpResponse response = Unirest.post("https://{yourDomain}/api/v2/tickets/password-change")
.header("content-type", "application/json")
.header("authorization", "Bearer MGMT_API_ACCESS_TOKEN")
