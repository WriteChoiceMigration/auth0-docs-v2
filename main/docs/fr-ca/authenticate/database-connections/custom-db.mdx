...
* **Les règles** s’exécutent lorsque des artefacts pour l’authenticité de l’utilisateur sont générés (c’est-à-dire un jeton d’ID dans <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">OpenID</Tooltip> Connect (OIDC)), un jeton d’accès dans <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">OAuth</Tooltip> 2.0 ou une assertion dans <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-2" tip="">Security Assertion Markup Language</Tooltip> (SAML).
