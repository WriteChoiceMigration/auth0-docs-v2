...
Si vous souhaitez récupérer l’une de ces propriétés avec le <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Management API</Tooltip> Auth0, incluez la permission `read:user_idp_tokens` lorsque vous [demandez un jeton d’accès](https://auth0.com/docs/secure/tokens/access-tokens/get-management-api-access-tokens-for-production).