...
<Frame>![Password Options](/images/fr-ca/cdy7uua7fh8z/1vqXxz0OD7hNLYLwrZZzIe/69da978ea21345366682968a270058a0/2025-02-25_10-33-44.png)</Frame>
...
Because password options are associated with a Database connection, you can access them using the <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">Management API</Tooltip>'s [Connections endpoints](/docs/fr-ca/api/management/v2#!/Connections). Password-related fields are stored in the `options` object. Because these fields are not used for non-database connections, they are not required, so if they are not enabled for a connection, they may not appear.
