...
Pour les implémentations d’authentification unique (<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">SSO</Tooltip>) initiées par le fournisseur de services, Auth0 est le fournisseur de services (SP) de SSO. Lorsqu’un utilisateur se connecte à une application :
...
* Auth0 fournit des instructions permettant de configurer les fournisseurs d’identité <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">SAML</Tooltip> suivants avec Auth0 :
...
* <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-2" tip="">OpenID</Tooltip> Connect (OIDC) ne prend pas en charge les authentifications uniques initiées par le fournisseur d’identité.
