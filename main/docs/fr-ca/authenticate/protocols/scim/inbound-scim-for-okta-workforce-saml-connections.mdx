...
Cette section décrit comment configurer une intégration <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">SAML</Tooltip> et une application SCIM personnalisées dans un nuage d’identité Okta Workforce, qui peut être utilisé pour la fourniture d’utilisateurs à votre point de terminaison Auth0 SCIM.
...
5. Passez en revue les **mappages supplémentaires** pour vous assurer que les attributs SCIM étendus sont mappés avec vos attributs Auth0 préférés, en examinant [mappage des attributs](/docs/authenticate/protocols/scim/configure-inbound-scim#attribute-mapping) pour plus de détails. Si vous recevez des numéros de téléphone et des adresses physiques, notez qu’Okta Workflow les envoie en tant que valeur`phoneNumbers[primary eq true].value` et `adresses[primary eq true].*` par défaut.
