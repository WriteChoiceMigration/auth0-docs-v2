...
Cette section décrit comment configurer une intégration <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">OpenID</Tooltip> Connect et une application SCIM personnalisées dans un nuage d’identité Okta Workforce, qui peut être utilisé pour la fourniture d’utilisateurs à votre point de terminaison Auth0 SCIM.

Pour plus d’informations sur la configuration d’une intégration <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">SAML</Tooltip> et SCIM avec le nuage d’identité Okta Workforce, consultez plutôt [SCIM entrant pour les connexions SAML d’Okta Workforce](/docs/fr-ca/authenticate/protocols/scim/inbound-scim-for-okta-workforce-saml-connections).
