...
Configurez Salesforce en tant que fournisseur d’identité <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">SAML</Tooltip> en procédant comme suit :
...
<td>Entrez le nom de domaine de messagerie à partir duquel vos utilisateurs se connecteront. Par exemple, si vos utilisateurs ont un domaine de messagerie <code>abc-example.com</code>, entrez-le dans ce champ. Vous pouvez entrer plusieurs domaines.</td>
</tr>
<tr>
<td><strong>URL de connexion</strong></td>
<td>Ouvrez le fichier de métadonnées que vous avez téléchargé depuis Salesforce et localisez la ligne qui contient la liaison <code>SingleSignOnService</code>. La valeur de l’attribut <code>location</code> sur cette ligne est votre URL de connexion. Elle devrait ressembler à quelque chose comme : <code>https://{sf-account-name}.my.salesforce.com/idp/endpoint/HttpRedirect</code> où <code>{sf-account-name}</code> est votre nom de domaine Salesforce.</td>
</tr>
<tr>
...
<td>Choisissez-en un avec <code>emailAddress</code></td>
</tr>
<tr>
<td><strong>Émetteur</strong></td>
<td><code>https://{your-saleforce-domain}.my.salesforce.com</code></td>
</tr>
</tbody>
</table>
...
Lors du dépannage de l’authentification unique, il est souvent utile de capturer une trace HTTP de l’interaction et de l’enregistrer dans un fichier HAR. Pour en savoir plus, lisez [Générer et analyser des fichiers HAR](/docs/troubleshoot/guides/generate-har-files).
...
Une fois le traceur http en place, capturez la séquence de connexion du début à la fin et analysez la trace pour trouver la séquence de GET. Une redirection de votre site d’origine vers l’<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">IDP</Tooltip> devrait s’afficher, ainsi qu’un affichage des identifiants, si vous avez dû vous connecter, puis une redirection vers l’URL de rappel. Le fichier HAR contiendra également la réponse SAML.
