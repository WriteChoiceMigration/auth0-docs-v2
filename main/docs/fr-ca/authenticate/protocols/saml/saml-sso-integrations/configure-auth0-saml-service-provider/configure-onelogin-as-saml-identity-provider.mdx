...
---
# Configurer OneLogin en tant que fournisseur d’identité SAML

Configurez OneLogin en tant que fournisseur d’identité <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">SAML</Tooltip> en effectuant les étapes suivantes :
...
* **<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">Audience</Tooltip> SAML** : `urn:auth0:{yourTenant}:yourConnectionName`
...
Enfin, veillez à activer la connexion initiée par l’<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-2" tip="">IdP</Tooltip> pour la connexion et à choisir l’application vers laquelle l’utilisateur est redirigé une fois l’assertion SAML consommée. Pour en savoir plus, lisez [Configurer l’authentification unique initiée par le fournisseur d’identité](/docs/fr-ca/authenticate/protocols/saml/saml-sso-integrations/identity-provider-initiated-single-sign-on).

## Modifier les mappages de connexion

Si vous utilisez OneLogin et Auth0, les utilisateurs qui se connectent à l’aide de OneLogin et qui sont créés dans <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-3" tip="">Auth0 Dashboard</Tooltip> ne disposeront pas de certaines informations de profil que vous souhaiteriez avoir.
