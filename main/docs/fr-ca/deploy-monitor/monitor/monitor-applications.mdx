...
Si vous utilisez les [règles](/docs/fr-ca/customize/rules) ou des [connexions personnalisées à la base de données](/docs/connections/database/custom-db) ou d’autres points d’extension, la ou les transactions synthétiques doivent être configurées de manière à utiliser les règles et/ou les scripts personnalisés de la base de données afin de garantir le bon fonctionnement de cet aspect du système.
...
* la plupart des appels à l’[Auth0 Management API](/docs/api/management/v2)
* les appels à [l’Authentication API](/docs/api/authentication) à partir d’applications Web classiques et d’applications de communication entre machines ([en savoir plus sur les types de clients](/docs/fr-ca/get-started/applications)).
