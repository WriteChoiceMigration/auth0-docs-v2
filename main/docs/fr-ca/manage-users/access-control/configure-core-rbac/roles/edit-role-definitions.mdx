...
---
# Modifier les définitions des rôles

Vous pouvez modifier un [rôle](/docs/fr-ca/manage-users/access-control/rbac) avec <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Auth0 Dashboard</Tooltip> ou <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">Management API</Tooltip>. Les rôles sont utilisés avec l’ensemble de fonctionnalités API Authorization Core.
...
Faites un appel `PATCH` au point de terminaison [Mettre à jour le rôle](/docs/api/management/v2#!/Roles/patch_roles_by_id). Assurez-vous de remplacer les valeurs des placeholders `ROLE_ID`, `MGMT_API_ACCESS_TOKEN`, `ROLE_NAME`, et `ROLE_DESC` par votre identifiant de rôle, votre jeton d’accès à Management API, le nom et la description de votre rôle, respectivement.
...
```
```java java.java
HttpResponse response = Unirest.patch("https://{yourDomain}/api/v2/roles/ROLE_ID")
.header("content-type", "application/json")
.header("authorization", "Bearer MGMT_API_ACCESS_TOKEN")
.header("cache-control", "no-cache")
...
<td><a href="https://auth0.com/docs/api/management/v2/tokens">Jeton d’accès pour Management API</a> avec la permission <code>update:roles</code>.</td>
</tr>
<tr>
