...
Vous pouvez utiliser le point de terminaison `POST /api/v2/jobs/users-exports` pour créer une tâche qui exporte tous les utilisateurs associés à une [connexion](/docs/identityproviders), ou tous les utilisateurs du locataire.
...
Vous aurez également besoin d’un [jeton d’accès à Management API](/docs/api/management/v2/tokens) valide.

## Créer le corps d’une demande

(Facultatif) Recherchez le `connection_id` et le nom de domaine de votre locataire Auth0 dans <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Auth0 Dashboard</Tooltip>. Créez un nouveau fichier texte avec le corps de la demande suivant :
...
```
```java java.java
HttpResponse response = Unirest.post("https://{yourDomain}/api/v2/jobs/users-exports")
.header("authorization", "Bearer {yourMgmtAPIAccessToken}")
.header("content-type", "application/json")
...
Une fois que vous avez créé votre job pour exporter vos utilisateurs, vous pouvez vérifier son statut en utilisant le [point de terminaison Get a Job](/docs/api/management/v2#!/Jobs/get_jobs_by_id).
...
```
```java java.java
HttpResponse response = Unirest.get("https://{yourDomain}/api/v2/jobs/%7ByourJobId%7D")
.header("authorization", "Bearer {yourMgmtAPIAccessToken}")
.asString();
```
