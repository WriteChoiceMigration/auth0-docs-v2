...
Les objets `event.session` et `api.session` prennent en charge les flux interactifs basés sur le Web, y compris le flux de code d’autorisation, le flux implicite, le flux de code d’appareil, ainsi que <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">SAML</Tooltip> et <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">WS-Fed</Tooltip>.
