...
---
# Mise à jour des profils utilisateur à l’aide de votre base de données

Mettez à jour les profils utilisateurs lorsque [vous utilisez votre propre base de données comme fournisseur d’identité](/docs/connections/database/custom-db) en effectuant les tâches suivantes :

* Utilisez [Management API](/docs/api/management/v2#!/Users/patch_users_by_id).
...
Lorsque vous utilisez votre propre base de données pour l’authentification, vous pouvez utiliser l[Management API](/docs/api/management/v2) pour mettre à jour les champs suivants :
...
Si vous avez [activé la migration des utilisateurs](/docs/connections/database/migrating), et qu’un utilisateur a déjà été migré vers la base de données Auth0, alors Auth0 n’interrogera plus votre base de données pour le profil utilisateur. Par conséquent, toutes les modifications apportées à la base de données personnalisée de cet utilisateur ne seront pas prises en compte dans Auth0.

Une fois qu’un utilisateur a été migré, vous pourrez également mettre à jour les champs tels que `name`, `nickname`, `given_name`, `family_name`, `picture`, `email` et `email_verified` par le biais de <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Management API</Tooltip>.
...
Auth0 met en cache le profil utilisateur reçu d’une [connexion à la base de données](/docs/connections/database) avant de l’envoyer à l’application cliente. Ce cache est stocké dans la base de données Auth0 et est actualisé à chaque fois que l’utilisateur s’authentifie.

Les valeurs mises en cache pour les champs du [Profil utilisateur normalisé](/docs/users/normalized/auth0/normalized-user-profile-schema) sont basées sur les valeurs renvoyées par le script de connexion à votre connexion de base de données personnalisée.
