...
1. Connectez l’utilisateur à votre application.
   ...
   L’utilisateur s’authentifie auprès de votre application à l’aide de la [connexion universelle](/docs/fr-ca/authenticate/login/auth0-universal-login). Pour en savoir plus, consultez [Démarrage rapide des applications Web ordinaires](/docs/quickstart/webapp), demandant un jeton pour le public de Management API Auth0 (audience=`https://{yourDomain}/api/v2/`).
2. Recherchez les utilisateurs dont l’adresse courriel est identique.
   ...
   Pour obtenir une liste de tous les enregistrements d’utilisateurs ayant la même adresse courriel, votre application appelle le point de terminaison [Get Users By Email endpoint](/docs/api/v2#!/users-by-email/get_users_by_email) en utilisant un [jeton d’accès Management API](/docs/fr-ca/secure/tokens/access-tokens/management-api-access-tokens) avec la permission `read:users`.
...
3. Invitez l’utilisateur à associer les comptes.
   ...
   2. Si l’utilisateur souhaite associer un compte donné, il peut cliquer sur **Associer** à côté du compte concerné.
      ...
      <Frame>![Example application with server-side account linking page](//images.ctfassets.net/cdy7uua7fh8z/7unQjUtxTZUc4tRFOxhaP3/050fb853d2b3be37795ce88b8a60ffed/account-linking-webapp-small__1_.png)</Frame>
...
5. Votre application appelle le point de terminaison [Associer un point de terminaison de compte utilisateur](/docs/api/v2#!/Users/post_identities) de l’API à l’aide d’un [Jeton d’accès Management API Access Token](/docs/fr-ca/secure/tokens/access-tokens/management-api-access-tokens) avec la permission `update:users`.
