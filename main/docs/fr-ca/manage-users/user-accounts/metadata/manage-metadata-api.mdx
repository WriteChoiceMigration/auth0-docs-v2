...
Effectuez l’appel `POST` suivant au point de terminaison <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Management API</Tooltip> [`/post_users`](/docs/api/management/v2#!/Users/post_users) pour créer l’utilisateur et définir les valeurs des propriétés :
...
Vous pouvez mettre à jour les métadonnées d’un utilisateur en effectuant un appel `PATCH` au point de terminaison [`/patch_users_by_id`](/docs/api/management/v2#!/Users/patch_users_by_id) de Management API.
