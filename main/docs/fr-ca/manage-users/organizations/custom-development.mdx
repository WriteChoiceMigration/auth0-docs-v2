...
Si l’utilisateur a été authentifié à l’aide d’une organisation et qu’une <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">audience</Tooltip> a été spécifié, le jeton d’accès sera un <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-2" tip="">JWT</Tooltip> et contiendra la demande `org_id` avec l’ID de l’organisation à laquelle l’utilisateur s’est connecté.
