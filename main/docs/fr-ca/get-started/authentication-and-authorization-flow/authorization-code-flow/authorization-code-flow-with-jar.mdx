...
Pour générer une requête JAR, vous devez d’abord créer un [jeton Web JSON (JWT)](/docs/fr-ca/secure/tokens/json-web-tokens). Utilisez la [bibliothèque Auth0 JWT](https://jwt.io/libraries) pour vous aider à générer un <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">JWT</Tooltip> dans votre langue préférée.
...
* `response_type`: indique à Auth0 le flux <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">OAuth</Tooltip> 2.0 que vous souhaitez exécuter. Utilisez `code` pour le flux d’octroi de code d’autorisation.
