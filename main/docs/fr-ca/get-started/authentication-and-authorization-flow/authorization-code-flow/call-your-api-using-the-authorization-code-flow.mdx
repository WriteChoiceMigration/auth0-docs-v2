...
Auth0 permet à votre application d’implémenter facilement le <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Code d’autorisation</Tooltip> flux à l’aide de(s) :

* [Guides de démarrage rapide pour applications Web classiques](/docs/quickstart/webapp) : la manière la plus simple d’implémenter le flux.
...
Pour autoriser l’utilisateur, votre application doit le rediriger vers [l’URL d’autorisation](/docs/api/authentication#authorization-code-grant).
...
<td>Jeton le type d’identifiant qu’Auth0 va retourner (<code>code</code> ou <code>token</code>). Pour ce flux, la valeur doit être <code>code</code>.</td>
</tr>
<tr>
...
Maintenant que vous avez un code d’authentification, vous pouvez l’échanger pour des jetons. En utilisant le code d’autorisation (`code`) extrait de l’étape précédente, vous devrez `POST` sur [l’URL du jeton](/docs/api/authentication#authorization-code).
...
<td>Définir sur <code>authorization_code</code>.</td>
</tr>
<tr>
<td><code>code</code></td>
<td><code>authorization_code</code> récupéré à l’étape précédente de ce tutoriel.</td>
</tr>
<tr>
<td><code>client_id</code></td>
...
<td>Le secret client de votre application. Vous pouvez trouver cette valeur dans les <a href="https://manage.auth0.com/#/Applications/{yourClientId}/settings">paramètres de votre application</a>.  Pour en savoir plus sur les méthodes d’authentification d’application disponibles, lisez <a href="/docs/secure/application-credentials">Identifiants d’applications</a>.</td>
</tr>
<tr>
<td><code>redirect_uri</code></td>
<td>L’URL de rappel valide définie dans les paramètres de votre Application. Cela doit correspondre exactement à <code>redirect_uri</code> passé à l’ URL d’autorisation à l’étape précédente de ce tutoriel. Notez que cela doit être codé URL.</td>
</tr>
</tbody>
</table>
...
Validez vos jetons avant de les enregistrer. Pour en savoir plus, lisez [Valider les jetons d’ID](/docs/secure/tokens/id-tokens/validate-id-tokens) et [Valider les jetons d’accès](/docs/secure/tokens/access-tokens/validate-access-tokens).

</Warning>

Les [Jetons d’ID](/docs/secure/tokens/id-tokens) contiennent des informations de l’utilisateur qui doivent être décodées et extraites.

Les [jetons d’accès](/docs/secure/tokens/access-tokens) sont utilisés pour appeler le [point de terminaison /userinfo de l’Authentication API Auth0](/docs/api/authentication#get-user-info) ou une autre API. Si vous appelez votre propre API, la première chose que votre API devra faire est de [vérifier le jeton d’accès](/docs/secure/tokens/access-tokens/validate-access-tokens).

Les [jetons d’actualisation](/docs/secure/tokens/refresh-tokens) sont utilisés pour obtenir un nouveau jeton d’accès ou un nouveau jeton d’ID après l’expiration du précédent. Le `refresh_token` ne sera présent dans la réponse que si vous avez inclus la permission `offline_access` et activé **Autoriser l’accès hors ligne** pour votre API dans le Dashboard.
...
Vous avez déjà reçu un [jeton d’actualisation](/docs/secure/tokens/refresh-tokens) si vous avez suivi ce tutoriel et complété ce qui suit :
