...
Si vous avez activé [Détection de mot de passe violé](/docs/fr-ca/secure/attack-protection/breached-password-detection) pour votre locataire, vous devez configurer votre application pour gérer les réponses de l’[Authentication API Auth0](/docs/api/authentication) en conséquence.
...
Votre application doit gérer cette erreur, afficher un message à l’utilisateur et [déclencher un flux de réinitialisation de mot de passe interactif](/docs/authenticate/database-connections/password-change#authentication-api).
