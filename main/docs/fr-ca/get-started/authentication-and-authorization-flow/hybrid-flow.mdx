...
6. L’application envoie le code d’autorisation, l’ID client de l’application et les identifiants de l’application, comme le secret client ou la clé privée JWT, au serveur d’autorisations Auth0 ([point de terminaison `/oauth/token`](/docs/api/authentication?http#authorization-code-flow43)).
