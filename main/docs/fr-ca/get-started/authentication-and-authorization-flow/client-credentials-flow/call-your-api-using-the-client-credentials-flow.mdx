...
Pour accéder à votre API, vous devez demander un jeton d’accès. Pour ce faire, vous devrez faire un `POST` vers le [jeton URL](/docs/api/authentication#client-credentials).
...
Pour un exemple de mise en œuvre, consultez le scénario d’architecture [Serveur Client + API](/docs/architecture-scenarios/application/server-api). Cette série de tutoriels est accompagnée d’un [exemple de code que vous pouvez consulter sur GitHub](https://github.com/auth0-samples/auth0-pnp-exampleco-timesheets).
