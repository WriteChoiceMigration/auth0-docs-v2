...
Les documents [OpenID Connect (OIDC) Discovery (Découverte OpenID Connect)](https://openid.net/specs/openid-connect-discovery-1_0-final.html#RFC5785)  contiennent des métadonnées sur le fournisseur d’identité (<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">IdP</Tooltip>). L’ajout de la découverte à votre trousse SDK pour pointer votre application vers le point de terminaison `./wellknown` pour consommer des informations sur votre IdP pourrait aider à configurer votre intégration avec l’IdP.
...
Vous pouvez configurer des applications avec les documents de découverte <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">OpenID</Tooltip> Connect (OIDC) trouvés ici : `https://{yourDomain}/.well-known/openid-configuration`.
...
Le logiciel médiateur OIDC ne prend pas en charge les <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-2" tip="">JWT</Tooltip> signés avec des clés symétriques. Assurez-vous de configurer votre application pour utiliser l’algorithme RSA à l’aide de clés publiques/privées.
