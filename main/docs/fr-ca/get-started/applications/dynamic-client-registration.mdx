...
Vous pouvez également mettre à jour cet indicateur en utilisant le point de terminaison de <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Management API</Tooltip> [`/Tenant/patch_settings`](/docs/api/management/v2#!/Tenants/patch_settings).
...
* **client_secret** : secret client alphanumérique de 64 bits. Cette valeur est utilisée par les applications pour s’authentifier auprès de l’Authentication API [`/token`](/docs/api/authentication#get-token) et pour signer et valider les jetons d’ID.
