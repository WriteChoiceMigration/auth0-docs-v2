...
* Nous avons utilisé la [Management API](/docs/api/management/v2#!/Users/patch_users_by_id) ou le Dashboard pour définir des informations spécifiques à l’application pour cet utilisateur.
...
* Les propriétés `favorite_color` et `user_metadata` ne sont pas présentes car <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">OpenID</Tooltip> Connect (OIDC) ne définit pas de demandes standard qui représentent `favorite_color` ou `user_metadata`.
