...
* **AI agents:** Control autonomous AI agent access to external APIs using <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">OAuth 2.0</Tooltip> scopes.
...
* **Comprehensive security controls:** Secures authentication flows with OAuth 2.0 and <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">OpenID</Tooltip> Connect and manages access for both human users and AI agents.
...
Leverage OAuth 2.0 and OpenID Connect to securely authenticate users with [Universal Login](/docs/fr-ca/authenticate/login/auth0-universal-login). With [Client-Initiated Account Linking](https://auth0.com/ai/docs/client-initiated-account-linking), AI agents can request access and connect to multiple <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-3" tip="">identity providers</Tooltip> on the user’s behalf through Auth0.

### Call APIs on users’ behalf

Using [Token Vault](/docs/fr-ca/secure/tokens/token-vault), AI agents can fetch <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-4" tip="">access tokens</Tooltip> for external identity providers to call their APIs, all through a single Auth0 integration. For example, you can use Token Vault to:
