...
Si vous décidez de personnaliser le script de la page de Connexion universelle, nous vous conseillons vivement d’utiliser le contrôle de version. Pour cela, vous devez déployer le script sur votre locataire Auth0 via [l’automatisation de déploiement](/docs/architecture-scenarios/b2c/deployment) ou via l’une des [stratégies alternatives](/docs/universal-login/version-control).
...
Avant de personnaliser vos modèles de courriels, veuillez configurer votre [Fournisseur de messagerie électronique](/docs/architecture-scenarios/b2c/operations#email-provider-setup).
