...
'og:description': "Autorisation des utilisateurs et considérations de planification liées à votre mise en œuvre IAM de commerce de détail (B2C)."
'og:image': "https://cdn2.auth0.com/docs/1.14567.0/img/share-image.png"
'twitter:title': "Autorisation (B2C)"
'twitter:description': "Autorisation des utilisateurs et considérations de planification liées à votre mise en œuvre IAM de commerce de détail (B2C)."
---
# Autorisation (B2C)
...
Auth0 prend en charge les restrictions d’accès pour les applications ou les API en fonction de certaines conditions. Dans certains scénarios, vous pourrez vouloir créer une règle qui renvoie une `UnauthorizedError` lorsque, par exemple, un utilisateur tente d’accéder à une application ou à une API à un moment inapproprié (comme décrit dans cet [exemple](/docs/authorization/sample-use-cases-rules-with-authorization#allow-access-only-on-weekdays-for-a-specific-application)), ou si l’utilisateur ne possède pas les bonnes demandes contenues dans ses `app_metadata`. Pour une application utilisant [OpenID Connect (OIDC)](/docs/fr-ca/authenticate/protocols/openid-connect-protocol), cela empêcherait l’attribution du [jeton d’ID](/docs/fr-ca/secure/tokens/id-tokens) utilisé pour autoriser l’accès. De même, pour une API, l’attribution de tout [jeton d’accès](/docs/fr-ca/secure/tokens/access-tokens) OAuth2 (utilisé lors de l’appel de l’API), pourrait être empêchée, comme décrit dans cet [exemple](/docs/authorization/sample-use-cases-rules-with-authorization#deny-access-to-anyone-calling-an-api).
...
Alternativement, vous pouvez avoir des données relatives à un utilisateur pour lesquelles des [fonctionnalités supplémentaires sont fournies](/docs/scopes/api-scopes#example-an-api-called-by-a-back-end-service) et pour lesquelles un consentement d’utilisateur explicite ne peut pas être obtenu (c’est-à-dire qu’il n’y a pas d’utilisateur authentifié qui puisse le fournir). Dans ce scénario, une [liste d’applications pour lesquelles l’octroi des identifiants client est activé peut être](/docs/fr-ca/get-started/authentication-and-authorization-flow/client-credentials-flow) définie.

### Demandes relatives aux jetons d’accès

Comme c’est le cas avec les jetons d’ID, vous pouvez [ajouter des demandes personnalisées aux jetons d’accès](/docs/scopes/sample-use-cases-scopes-and-claims#add-custom-claims-to-a-token) en utilisant l’extensibilité des règles Auth0. Une fois ajoutée, votre API peut ensuite vérifier un jeton d’accès pour les demandes nécessaires et soit autoriser, soit empêcher l’accès à certaines fonctionnalités selon les besoins.
...
Avec <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-2" tip="">FGA</Tooltip>, vous pouvez créer un modèle d’autorisation pour déterminer la relation avec laquelle vous souhaitez déterminer l’accès des utilisateurs.

## RBAC (Contrôle d’accès basé sur les rôles)

Auth0 permet le contrôle d’accès basé sur les rôles([RBAC](/docs/fr-ca/manage-users/access-control/rbac)). RBAC fait référence à l’attribution d’autorisation aux utilisateurs en fonction de leur rôle au sein d’une organisation, et offre un contrôle d’accès plus simple en proposant une approche plus gérable, moins sujette à erreur.

## Autorisation machine-machine (M2M)

Il existe de nombreux scénarios qui nécessitent qu’une application sans session utilisateur interactive obtienne un jeton d’accès afin d’appeler une API. Dans de tels scénarios, vous devez authentifier le client au lieu de l’utilisateur, et <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-3" tip="">OAuth</Tooltip> 2 fournit le type d’octroi des [identifiants client](/docs/fr-ca/get-started/authentication-and-authorization-flow/client-credentials-flow) pour faciliter le processus. Quelques exemples courants où cela est nécessaire :
