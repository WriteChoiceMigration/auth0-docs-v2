...
'description': "Vérifications des opérations à effectuer avant le lancement de votre implémentation IAM B2B."
'og:title': "Préparation des opérations (B2B)"
'og:description': "Vérifications des opérations à effectuer avant le lancement de votre implémentation IAM B2B."
'og:image': "https://cdn2.auth0.com/docs/1.14567.0/img/share-image.png"
'twitter:title': "Préparation des opérations (B2B)"
'twitter:description': "Vérifications des opérations à effectuer avant le lancement de votre implémentation IAM B2B."
---
...
En outre, vous devez consulter régulièrement [la page des migrations Auth0](/docs/fr-ca/troubleshoot/product-lifecycle/deprecations-and-migrations) pour obtenir des informations sur les dépréciations à venir qui pourraient obliger votre équipe à effectuer des changements.
...
* L’[outil Auth0 Deploy CLI](/docs/fr-ca/deploy-monitor/deploy-cli-tool) vous offre un script facile d’utilisation qui s’intègre à votre pipeline d’intégration continue/déploiement continu (CI/CD) existant.
* Les [extensions du contrôle de code source](/docs/fr-ca/customize/extensions) Auth0 fournissent un processus d’automatisation de base facile à configurer avec très peu de maintenance si vous n’avez pas de pipeline CI/CD, ou dans les cas où vous ne pouvez pas intégrer directement un pipeline CI/CD.
* Si votre flux de travail de développement utilise [Terraform](https://www.terraform.io/), le [fournisseur Terraform Auth0](/docs/fr-ca/deploy-monitor/auth0-terraform-provider) vous aide à gérer les configurations de vos locataires Auth0.
...
* Vérifiez que vous utilisez la dernière version prise en charge de node.js dans [Paramètres Auth0 Dashboard](/docs/fr-ca/get-started/tenant-settings).
* Vérifiez que vous utilisez une version de la trousse SDK/bibliothèques prise en charge par Auth0 conformément à la [Matrice d’assistance d’Auth0](/docs/fr-ca/troubleshoot/customer-support/product-support-matrix).
