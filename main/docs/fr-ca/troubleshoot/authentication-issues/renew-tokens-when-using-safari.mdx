...
L’activation de l’ITP fait que le navigateur se comporte comme si vous aviez désactivé les témoins tiers dans le navigateur, ce qui rend impossible l’obtention d’un nouveau jeton sans afficher quoi que ce soit à l’utilisateur. Cela s’apparente à la manière dont <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">OpenID</Tooltip> Connect (OIDC) utilise les iframes pour gérer les [sessions](/docs/fr-ca/manage-users/sessions) dans les applications à page unique.
