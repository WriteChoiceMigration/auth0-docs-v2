...
Les liens de réinitialisation du mot de passe et de vérification par courriel ne seront plus enregistrés dans les journaux des locataires. Ces liens peuvent être récupérés à partir de la réponse à la demande de <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Management API</Tooltip> de réinitialisation du mot de passe ou de la vérification par courriel.
...
* Auth0 a restreint la personnalisation/modification des demandes <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-2" tip="">OPENID</Tooltip> standard ou des demandes utilisées par Auth0 en interne.
...
* [`GET /api/v2/clients`](/docs/api/management/v2#!/Clients/get_clients)
* [`GET /api/v2/client-grants`](/docs/api/management/v2#!/Clients/client_grants)
* [`GET /api/v2/grants`](/docs/api/management/v2#!/Clients/grants)
* [`GET /api/v2/connections`](/docs/api/management/v2#!/Clients/connections)
* [`GET /api/v2/device-crecentials`](/docs/api/management/v2#!/Clients/device_credentials)(lorsque le paramètre de requête `type` est utilisé)
* [`GET /api/v2/resource-servers`](/docs/api/management/v2#!/Clients/resource_servers)
* [`GET /api/v2/rules`](/docs/api/management/v2#!/Clients/rules)
...
Le 18 mai 2021, dans le nuage public et dans la version de juin du nuage privé (v2106), Auth0 commencera à utiliser la propriété de connexion pour toutes les connexions Azure AD et ADFS. Vous devez vous assurer que toutes vos connexions sont correctement configurées avant cette date. Pour en savoir plus, consultez [Vérification des courriels pour Azure AD et ADFS](/docs/connections/azuread-adfs-email-verification).
...
Le 31 mai 2017, dans le cadre des initiatives d’Auth0 en matière d’amélioration de la qualité, nous avons ajouté la possibilité d’exécuter des règles pendant l’échange d’octroi de mot de passe de propriétaire de ressource <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-3" tip="">OAuth</Tooltip> 2.0 (l’échange de mot de passe) et l’échange de jeton d’actualisation.
...
Si vous souhaitez activer le nouveau comportement sur ce locataire pour tester la date d’activation obligatoire, connectez-vous à [Dashboard](https://manage.auth0.com/#) et activez la bascule **Run Rules on Password and <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-4" tip="">Refresh Token</Tooltip> Exchanges (Exécuter des règles sur les échanges de mots de passe et de jetons d’actualisation)** dans  [Tenant Settings (Paramètres du locataire) > Advanced (Avancés)](https://manage.auth0.com/#/tenant/advanced).
