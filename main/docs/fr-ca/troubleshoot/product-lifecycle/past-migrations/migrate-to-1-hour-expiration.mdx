...
* Nous vous recommandons de [configurer des chemins de connexion par défaut](https://auth0.com/docs/authenticate/login/auth0-universal-login/configure-default-login-routes) pour vos applications dans <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Auth0 Dashboard</Tooltip> ou via Auth0 Management API. Cette méthode lance automatiquement les transactions de connexion et offre la meilleure expérience à l’utilisateur, car le flux de connexion redémarre de manière transparente pour les utilisateurs finaux actifs lorsque la transaction a expiré.
