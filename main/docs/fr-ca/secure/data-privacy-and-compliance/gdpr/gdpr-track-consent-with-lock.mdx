...
Au lieu de créer une application de zéro, nous allons [utiliser l’exemple Guide rapide JavaScript](/docs/quickstart/spa/vanillajs) d’Auth0. Nous utiliserons d’autre part les pages <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Universal Login</Tooltip> (Connexion universelle) d’Auth0 afin de pouvoir implémenter une expérience de connexion universelle au lieu d’intégrer la connexion dans notre application. Pour en savoir plus sur la connexion universelle, consultez [Connexion universelle Auth0](/docs/fr-ca/authenticate/login/auth0-universal-login). Pour en savoir plus sur les différences entre la connexion universelle et la connexion intégrée, consultez [Connexion universelle centralisée par rapport à la Connexion intégrée](/docs/fr-ca/authenticate/login/universal-vs-embedded-login).
...
6. [Téléchargez l’exemple d’application JavaScript à page unique](/docs/quickstart/spa/vanillajs).
