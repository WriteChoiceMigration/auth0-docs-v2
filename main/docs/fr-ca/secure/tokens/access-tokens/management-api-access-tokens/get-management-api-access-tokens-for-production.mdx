...
Pour demander à Auth0 un jeton de <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Management API</Tooltip> v2, effectuez une opération `POST` vers le point de terminaison `https://{yourDomain}/oauth/token` en utilisant les identifiants de l’application machine-machine que vous avez créée dans l’étape préalable.
...
<td>Indique le <a href="/docs/protocols/oauth2#grant-types">flux OAuth 2.0</a> que vous souhaitez exécuter. Pour la communication entre machines, utilisez la valeur <code>client_credentials</code>.</td>
</tr>
<tr>
...
<td>Il s’agit de la valeur du champ <strong>Identifier</strong> de l’<code>Auth0 Management API</code>. Vous pouvez le trouver dans l’onglet <a href="https://manage.auth0.com/#/api">Paramètres de l’API</a>.</td>
</tr>
</tbody>
</table>
...
Par exemple, pour [obtenir toutes les applications](/docs/api/management/v2#!/Clients/get_clients) procédez comme suit :
...
Ce script python obtient un jeton d’accès à Management API v2, l’utilise pour appeler le point de terminaison [Obtenir toutes les applications](/docs/api/management/v2#!/Clients/get_clients) et affiche la réponse dans la console.
