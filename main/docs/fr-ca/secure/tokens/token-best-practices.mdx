...
Les jetons d’actualisation s’accumulent en raison des tests automatisés et sont généralement utilisés pendant toute la durée du test. Pour éviter une accumulation de jetons soumise à des limites de jetons d’actualisation, utilisez l’Auth0 <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-3" tip="">Management API</Tooltip> pour supprimer les jetons d’actualisation inutiles.
