...
Le jeton d’ID d’un utilisateur authentifié peut être utilisé avec le point de terminaison `/delegation` pour demander un jeton de délégation pour une cible particulière. La cible peut être un module complémentaire d’une application configurée dans Auth0. Les modules complémentaires pour lesquels cela est possible sont ceux qui ne sont pas des modules complémentaires <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">SAML</Tooltip> ou <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">WS-Fed</Tooltip>. Le module complémentaires doit être configuré dans Auth0 avec des secrets obtenus auprès du service de module complémentaire, tel que Firebase. Les instructions relatives à la configuration des secrets sont accessibles sur la page de configuration de chaque module complémentaires. Les secrets sont utilisés pour signer le jeton de délégation afin que l’API du module complémentaires puisse valider le jeton et lui faire confiance.
