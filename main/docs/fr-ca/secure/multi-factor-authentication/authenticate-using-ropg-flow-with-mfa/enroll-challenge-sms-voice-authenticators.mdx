...
Auth0 offre un flux intégré d’enregistrement et d’authentification <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">MFA</Tooltip> en utilisant la [connexion universelle](/docs/fr-ca/authenticate/login/auth0-universal-login). Cependant, si vous souhaitez créer votre propre interface utilisateur, vous pouvez utiliser [l’API MFA](/docs/fr-ca/secure/multi-factor-authentication/multi-factor-authentication-developer-resources/mfa-api) pour ce faire.
...
* [Configurez le téléphone comme facteur d’authentification](/docs/fr-ca/secure/multi-factor-authentication/multi-factor-authentication-factors/configure-sms-voice-notifications-mfa) dans Dashboard ou à l’aide de la [Management API](/docs/api/management/v2#!/Guardian/put_factors_by_name).
...
<td><code>[sms]</code> ou <code>[voice]</code></td>
</tr>
<tr>
<td><code>phone_number</code></td>
<td><code>+11...9</code>, numéro de téléphone <a href="https://en.wikipedia.org/wiki/e.164">format E.164</a></td>
</tr>
</tbody>
</table>
...
Pour compléter l’inscription, réalisez une requête `POST` au point de terminaison <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">OAuth</Tooltip> Token. Vous devez inclure le `oob_code` retourné dans la réponse précédente et le `binding_code` avec la valeur reçue dans le message.
