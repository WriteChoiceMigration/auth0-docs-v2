...
---
# Clés d’identification natives pour applications mobiles
...
Pour préparer votre application aux clés d’identification natives, vous devez configurer les paramètres de votre appareil et ajouter l’autorisation `Passkey (Clé d’identification)`. Vous pouvez terminer ces configurations dans votre <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Auth0 Dashboard</Tooltip> ou en utilisant <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">Management API</Tooltip>.
...
Appelez le point de terminaison [Update a Client (Mettre à jour un client)](/docs/api/management/v2/clients/patch-clients-by-id) et :
...
2. Votre application lance ensuite le défi-réponse d’inscription en appelant le point de terminaison [Demander un défi-réponse d’inscription](/docs/native-passkeys-api#request-signup-challenge) d’Auth0 Authentication API :

   <Note>
   * Si vous ne spécifiez pas de `realm`, le répertoire par défaut de votre locataire est utilisé.
   * Par défaut, le courriel est l’identifiant requis. Si vous avez activé [Identifiants flexibles](/docs/authenticate/database-connections/activate-and-configure-attributes-for-flexible-identifiers) pour votre connexion de base de données, vous pouvez utiliser une combinaison de `email`, `phone_number`, ou `username` à la place. Ces options peuvent être obligatoires ou facultatives et doivent correspondre à votre configuration d’identificateur flexible.</Note>
...
5. Votre application utilise ensuite les identifiants obtenus lors du processus d’enregistrement, y compris les détails `authn_response`, pour appeler le point de terminaison [Jeton](/docs/native-passkeys-api#authenticate-new-user) :
...
1. Un utilisateur lance votre application et commence le processus de connexion. Votre application lance ensuite le défi-réponse de connexion en appelant le point de terminaison[Demander un défi-réponse de connexion](/docs/native-passkeys-api#request-login-challenge) d’Authentication API :
...
4. Votre application utilise ensuite les identifiants obtenus lors du processus de connexion, y compris les détails `authn_response`, pour appeler le point de terminaison[Jeton](/docs/native-passkeys-api#authenticate-existing-user) :
...
* Auth0 Authentication API (API d’authentification Auth0)
  ...
  + [Demander un défi-réponse d’inscription](/docs/native-passkeys-api#request-signup-challenge)
  + [Authentifier un nouvel utilisateur](/docs/native-passkeys-api#authenticate-new-user)
  + [Demander un défi-réponse de connexion](/docs/native-passkeys-api#request-login-challenge)
  + [Authentifier un utilisateur existant](/docs/native-passkeys-api#authenticate-existing-user)
* Ressources Android
