...
---
# Mettre à jour des secrets d’appels
...
Vous pouvez mettre à jour les secrets d’appels ajoutés à un appel donné à l’aide du Dashboard ou de <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Management API</Tooltip>.
...
Effectuez un appel `PATCH` au point de terminaison[Update Hook Secrets](/docs/api/management/v2/#!/Hooks/patch_secrets). Veillez à remplacer les valeurs de remplacement `HOOK_ID` et `MGMT_API_ACCESS_TOKEN` par votre identifiant d’appel et votre jeton d’accès à Management API, respectivement.
...
```
```java java.java
HttpResponse response = Unirest.patch("https://{yourDomain}/api/v2/hooks/HOOK_ID/secrets")
.header("content-type", "application/json")
.header("authorization", "Bearer MGMT_API_ACCESS_TOKEN")
.header("cache-control", "no-cache")
