...
---
# Créer des secrets de hook d’appel
...
Faites une requête `POST` au [point de terminaison Ajouter des Secrets d’appel](/docs/api/management/v2/#!/Hooks/post_secrets). Assurez-vous de remplacer les valeurs des paramètres fictifs `MGMT_API_ACCESS_TOKEN`, `HOOK_ID`, `HOOK_SECRET_KEY` et `HOOK_SECRET_VALUE` avec votre jeton d’accès de Management  API, votre identifiant de hookID d’appel et les paires clé-valeur de hook d’appel respectivement.
