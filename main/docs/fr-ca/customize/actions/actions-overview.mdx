...
<Accordion title="Inscription et connexion Lorsqu’un utilisateur est ajouté à une base de données ou à une connexion sans mot de passe.">
...
<Accordion title="Notifications MFA Lorsque vous utilisez SMS comme facteur pour l’authentification multifacteur (MFA) ou pour configurer un fournisseur personnalisé.">
...
<Accordion title="Réinitialisation du mot de passe Lorsque la réinitialisation du mot de passe est lancée.">
...
<Accordion title="Communication entre machines Lorsqu’une application, et non un utilisateur, est authentifiée et autorisée à utiliser vos API.">
...
* Lorsque vous modifiez une Action dans le <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Auth0 Dashboard</Tooltip>, vous avez accès à de riches informations sur les types et à une documentation en ligne sur les possibilités offertes par chaque déclencheur, qui vous permet de découvrir facilement les capacités prises en charge par chaque Déclencheur.
