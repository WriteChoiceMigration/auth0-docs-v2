...
* Les appels effectués vers <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Management API</Tooltip> Auth0 et les mises à jour des métadonnées utilisateur sont [limités](/docs/fr-ca/troubleshoot/customer-support/operational-policies/rate-limit-policy/management-api-endpoint-rate-limits).
...
* Un nouvel objet `event.request` est émis à chaque fois qu’un flux d’Action est suspendu, puis repris par la suite (par exemple, en raison d’une redirection ou d’un défi <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">MFA</Tooltip>).
...
* Un maximum de 100 attributs <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-2" tip="">SAML</Tooltip> peut être modifié ou ajouté par des Actions.
...
* La récupération des jetons <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-3" tip="">IdP</Tooltip> externes à partir du tableau Identities (Identités)
