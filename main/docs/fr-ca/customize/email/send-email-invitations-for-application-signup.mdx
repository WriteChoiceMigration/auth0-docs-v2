...
Vous pouvez autoriser un utilisateur à accéder à un compte existant que vous avez créé en son nom. Ensuite, envoyez à l’utilisateur un lien unique pour qu’il puisse définir son mot de passe. Vous générez le lien unique en créant un ticket de modification de mot de passe dans lequel votre application d’invitation appelle le point de terminaison `/password-change`[Management API](/docs/api/management/v2/#!/Tickets/post_password_change). Vous aurez besoin de :

* [Créer un utilisateur de la base de données Auth0](/docs/fr-ca/authenticate/database-connections) avec le paramètre `user.email_verified` défini sur `false`. Vous pouvez utiliser le point de terminaison [Créer un utilisateur](/docs/api/management/v2/#!/Users/post_users).
