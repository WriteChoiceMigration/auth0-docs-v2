...
---
# Description des modèles de courriel
...
Pour certaines erreurs, l’utilisateur peut être envoyé vers la page [Erreur générique](/docs/authenticate/login/auth0-universal-login/error-pages) au lieu de l’URL de redirection.

</Note>

Vous pouvez éviter l’inclusion des adresses courriel dans les liens de redirection en mettant à jour les modèles de courriel via <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Management API</Tooltip>. Par exemple :
...
Ce courriel sera généré lors de l’envoi d’une invitation d’inscription à <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">MFA</Tooltip>. Le message contiendra un lien qui, lorsqu’il sera consulté, affichera l’expérience d’inscription MFA.
...
Si l’utilisateur n’accepte pas une invitation avant qu’elle n’expire, l’invitation sera marquée comme expirée et éventuellement supprimée d’[Auth0 Dashboard](/docs/get-started/auth0-overview/dashboard).
