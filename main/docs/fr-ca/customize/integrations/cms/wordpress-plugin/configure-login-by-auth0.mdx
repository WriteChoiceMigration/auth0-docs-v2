...
L’URL de rappel **ne doit pas** être mise en cache, sinon une erreur « État non valide » risque de se produire lors de la connexion. Pour en savoir plus, consultez [Dépannage des erreurs d’état non valide des modules d’extension WordPress](/docs/customize/integrations/cms/wordpress-plugin/troubleshoot-wordpress-plugin-invalid-state-errors).
...
* **Algorithme de signature <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-2" tip="">JWT</Tooltip>** L’algorithme utilisé pour signer les jetons à partir de l’onglet Paramètres avancés de l’application, <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-3" tip="">OAuth</Tooltip>; par défaut est RS256. Le nom de l’option est `client_signing_algorithm`.
...
* **Page de connexion universelle** : Redirige la page `wp-login.php` vers la page de connexion universelle pour l’authentification unique <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-4" tip="">SSO</Tooltip> en utilisant toutes les connexions actives pour cette application. Le nom de l’option est `auto_login`.
