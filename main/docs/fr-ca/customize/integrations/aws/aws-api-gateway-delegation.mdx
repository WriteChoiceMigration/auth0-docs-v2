...
Le guide vous montre ensuite comment configurer Amazon API Gateway en utilisant les fonctions AWS Lambda, en sécurisant ces fonctions avec les rôles IAM AWS, puis en obtenant un jeton pour le rôle IAM AWS à l’aide de la délégation Auth0. Il vous montrera ensuite comment attribuer différentes autorisations à diverses classes d’utilisateurs, comme la base de données interne ou les utilisateurs sociaux, et comment faire circuler l’identité à l’aide d’un jeton Web JSON (<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">JWT</Tooltip>).
