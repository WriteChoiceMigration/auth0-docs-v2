...
Dans AWS, créez un nouveau fournisseur d’identité (<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">IdP</Tooltip>) :
...
Cognito utilise la clé de connexion publique [Métadonnées du fournisseur OpenID](https://subscription.auth0.com/.well-known/jwks.json) pour valider la signature du jeton Web JSON (<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">JWT</Tooltip>).
