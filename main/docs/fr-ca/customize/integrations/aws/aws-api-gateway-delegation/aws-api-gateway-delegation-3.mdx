...
À ce stade, vous avez configuré l’authentification avec Auth0 et vous disposez d’un <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">JWT</Tooltip> <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">OpenID</Tooltip>. Voici la structure du répertoire pour le code généré :

<Frame>![AWS API Gateway - project directory](//images.ctfassets.net/cdy7uua7fh8z/5fYL14NE1L9WuzUyA0xTky/35e78ca14e6d0c5b36582b5b74a6fc55/aws-api-gateway-project.png)</Frame>

Vous pouvez exploiter la fonctionnalité de délégation d’Auth0 pour obtenir un jeton d’accès AWS à partir du jeton d’identité Auth0. En arrière-plan, Auth0 authentifie votre jeton d’identité, puis utilise <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-2" tip="">SAML</Tooltip> en fonction du module complémentaire que vous avez configuré.
...
Pour examiner le téléchargement, veuillez consulter [Tutoriel sur la passerelle API AWS, deuxième partie](/docs/customize/integrations/aws/aws-api-gateway-delegation/aws-api-gateway-delegation-2#deploy-the-api).
