...
* Transmettre l’identité au service en passant votre jeton Web JSON <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">OpenID</Tooltip> (<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">JWT</Tooltip>);
* Valider le jeton;
...
Une façon d’ajouter l’adresse courriel d’un utilisateur au JWT est d’utiliser une [règle](/docs/rules). Il s’agit d’une bonne approche si vous voulez vous assurer que cette valeur est toujours disponible dans le JWT pour un utilisateur qui s’authentifie.
...
Mettez à jour `auth0-variables.js` avec votre clé secrète, qui peut être trouvée dans l’onglet Paramètres de votre application dans l’<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-2" tip="">Auth0 Dashboard</Tooltip> :
...
* Intégré un fournisseur d’identités <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-3" tip="">SAML</Tooltip> avec IAM pour associer l’accès à l’API à votre base d’utilisateurs.
