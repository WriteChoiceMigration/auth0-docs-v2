...
Ajoutez une méthode OPTIONS pour la ressource `purchase`, comme indiqué précédemment pour les `pets` dans la [Section Configurer CORS et déployer l’API de l’étape 2 - Sécuriser et déployer la passerelle API Amazon](/docs/customize/integrations/aws/aws-api-gateway-delegation/aws-api-gateway-delegation-2#set-up-cors-and-deploy-the-api).
...
Une fois que vous avez défini la fonction Lambda, [ajouter une méthode POST à la ressource `purchase`](/docs/customize/integrations/aws/aws-api-gateway-delegation/aws-api-gateway-delegation-1#method-post-pet-information) qui appelle le `PetPurchase` Lambda Assurez-vous d’ajouter également l’en-tête `Access-Control-Allow-Origin` avec une valeur de `*` à la méthode POST en utilisant la configuration de réponse de la méthode/réponse d’intégration que l’on trouve dans la [Section Configurer CORS et déployer l’API de l’étape 2 - Sécuriser et déployer la passerelle API Amazon](/docs/customize/integrations/aws/aws-api-gateway-delegation/aws-api-gateway-delegation-2#set-up-cors-and-deploy-the-api).
...
En utilisant la [Console de passerelle Amazon API](https://console.aws.amazon.com/apigateway), vous allez à nouveau [déployer l’API et générer une nouvelle trousse SDK JavaScript](/docs/customize/integrations/aws/aws-api-gateway-delegation/aws-api-gateway-delegation-2#set-up-cors-and-deploy-the-api).
