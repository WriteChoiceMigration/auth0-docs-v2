...
---
# Exporter des données utilisateur vers Marketo
...
```
```java java.java
HttpResponse response = Unirest.post("https://marketo_rest_api_base_url/bulk/v1/leads.json")
.header("authorization", "Bearer {MARKETO_ACCESS_TOKEN}")
...
Vous pouvez vérifier l’état de votre importation en utilisant l’API [Obtenir l’état de l’importation d’un prospect](/docs/)et le `batchId` de votre tâche d’importation. Par exemple :
...
```
```java java.java
HttpResponse response = Unirest.get("https://marketo_rest_api_base_url/bulk/v1/leads/batch/BATCH_ID.json")
.header("authorization", "Bearer {MARKETO_ACCESS_TOKEN}")
.asString();
```
