...
Différentes valeurs sont prises en charge pour le paramètre scope. Gardez à l’esprit que les <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">JWT</Tooltip> sont envoyés sur chaque demande d’API, il est donc souhaitable de les garder aussi petits que possible.
