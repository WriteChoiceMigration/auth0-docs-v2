...
Permission utilisée pour l’authentification. La valeur par défaut est `openid`. Elle retournera non seulement le **Jeton d’accès**, mais aussi un **Jeton d’ID** qui est un jeton Web JSON (<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">JWT</Tooltip>) contenant des informations sur l’utilisateur. Veuillez consulter la documentation sur les [Permissions](/docs/fr-ca/get-started/apis/scopes) pour plus d’informations sur les permissions d’authentification.
