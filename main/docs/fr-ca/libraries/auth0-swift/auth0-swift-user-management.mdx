...
Unlinking users is a similar process to linking users. The `unlink` method takes three parameters: the secondary profile’s user ID, the secondary profile’s provider (the connection’s <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">identity provider</Tooltip>), and the primary profile’s user ID.
