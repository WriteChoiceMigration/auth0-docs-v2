...
Utilisez le sélecteur interactif pour créer une nouvelle application Auth0 ou sélectionnez une application existante qui représente le projet que vous souhaitez intégrer. Dans Auth0, il est attribué à chaque application un ID client unique alphanumérique que votre code d’application utilise pour appeler les API Auth0 via la trousse SDK.
...
De même, vous devez créer une nouvelle API Auth0 ou utiliser une API existante qui représente le projet que vous intégrez à partir du [Dashboard](https://manage.auth0.com/#/). Choisissez un identifiant unique pour l’API et notez-le. Cet identifiant vous sera nécessaire pour configurer votre application ci-dessous.

[2

## Installer la trousse SDK Auth0 pour PHP](#installer-la-trousse-sdk-auth0-pour-php)

Auth0 fournit une trousse [SDK pour PHP](https://github.com/auth0/auth0-PHP) (Auth0-PHP) pour simplifier le processus de mise en œuvre de l’authentification et de l’autorisation Auth0 dans les applications PHP.

La trousse SDK Auth0 pour PHP nécessite l’installation des bibliothèques HTTP compatibles [PSR-17](https://www.php-fig.org/psr/psr-17/) et [PSR-18](https://www.php-fig.org/psr/psr-18/) pour la gestion des requêtes réseau. Si vous ne disposez pas de bibliothèques disponibles, vous pouvez installer des choix fiables en exécutant les commandes suivantes dans votre terminal :
...
Installez maintenant la trousse SDK Auth0 pour PHP en exécutant la commande suivante dans votre terminal :
...
* `strategy` : La stratégie aide à orienter le comportement de la troousse SDK en ce qui concerne l’utilisation de votre application. Dans ce cas, vous souhaitez la définir sur la constante
...
* clientId : L’identifiant de l’application Auth0 que vous avez configurée précédemment dans le démarrage rapide. Celui-ci figure dans Auth0 Dashboard, sous les paramètres de votre application, dans le champ ID client.
...
* public : L’identifiant de l’API Auth0 que vous avez enregistré ci-dessus. Il doit être fourni sous la forme d’un tableau.
...
La méthode la plus simple pour récupérer les jetons d’accès à partir d’une requête est la méthode `getBearerToken()` de la trousse SDK pour PHP. Cette méthode récupère les jetons à partir des paramètres GET, des corps POST, des en-têtes de requête et d’autres sources. Dans ce cas, la trousse SDK pour PHP traite les jetons transmis par les requêtes GET dans le paramètre `token` ou dans l’en-tête HTTP `Authorization`.
