...
'og:title': "Login"
'og:description': "This tutorial demonstrates how to call your API from an input-constrained device using the Device Authorization flow."
'og:image': "https://cdn2.auth0.com/docs/1.14567.0/media/platforms/device.svg"
'twitter:title': "Login"
'twitter:description': "This tutorial demonstrates how to call your API from an input-constrained device using the Device Authorization flow."
---

# Device Authorization Flow

<Frame>![Gravatar for rachel.khoriander@auth0.com](//www.gravatar.com/avatar/36c8a1120563bcbdb2ebdb2ab63b7139?d=mm&r=g&s=50)</Frame>
...
Log In & Download Sample[View on Github](https://github.com/undefined/undefined/tree/master/01-Login)

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">

This tutorial will help you call your own API from an input-constrained device using the Device Authorization Flow. If you want to learn how the flow works and why you should use it, see [Device Authorization Flow](/docs/fr-ca/flows/concepts/device-auth).
...
* [Register the Application with Auth0](/docs/fr-ca/dashboard/guides/applications/register-app-native).
  ...
  + Make sure the Application's **[Grant Types](/docs/fr-ca/dashboard/guides/applications/update-grant-types)** include **Device Code**. This is also in the [Dashboard](https://manage.auth0.com/#), under **Application Settings > Advanced > Grant Types**.
  + If you want your Application to be able to use [Refresh Tokens](/docs/fr-ca/tokens/concepts/refresh-tokens), make sure the Application's **[Grant Types](/docs/fr-ca/dashboard/guides/applications/update-grant-types)** include **Refresh Token**.
* Set up and enable at least one connection for the Application: [Database connections](/docs/fr-ca/dashboard/guides/connections/set-up-connections-database), [Social connections](/docs/fr-ca/dashboard/guides/connections/set-up-connections-social)
* [Register your API with Auth0](/docs/fr-ca/architecture-scenarios/mobile-api/part-2#create-the-api)

  + If you want your API to receive [Refresh Tokens](/docs/fr-ca/tokens/concepts/refresh-tokens) to allow it to obtain new tokens when the previous ones expire, enable **Allow Offline Access**.
* [Configure Device User Code Settings](/docs/fr-ca/dashboard/guides/tenants/configure-device-user-code-settings) to define the character set, format, and length of your randomly-generated user code.
...
To get the device code, your app must request a code from the [device code URL](/docs/fr-ca/api/authentication#get-device-code), including the Client ID.
...
<tr>
<td>`scope`</td>
<td>The <a href="/docs/fr-ca/scopes">scopes</a> for which you want to request authorization. These must be separated by a space. You can request any of the <a href="https://openid.net/specs/openid-connect-core-1_0.html#StandardClaims">standard OIDC scopes</a> about users, such as `profile` and `email`, <a href="/docs/fr-ca/tokens/concepts/jwt-claims#custom-claims">custom claims</a> conforming to a <a href="/docs/fr-ca/tokens/guides/create-namespaced-custom-claims">namespaced format</a>, or any <a href="/docs/fr-ca/scopes/current/api-scopes">scopes supported by the target API</a> (e.g., `read:contacts`). Include `openid` to get an ID Token or to be able to use the <a href="/docs/fr-ca/api/authentication#user-profile">/userinfo endpoint</a> to retrieve profile information for the user. Include `offline_access` to get a Refresh Token (make sure that the <strong>Allow Offline Access</strong> field is enabled in the <a href="https://manage.auth0.com/#/apis">API Settings</a>). Note that this must be URL encoded.</td>
</tr>
<tr>
<td>`audience`</td>
...
You can [configure the character set, format, and length of your randomly-generated user code](/docs/fr-ca/dashboard/guides/tenants/configure-device-user-code-settings) in your tenant settings.
...
<Frame>![Request Device Activation](https://cdn2.auth0.com/docs/1.14567.0/media/articles/flows/guides/device-auth/request-device-activation.png)</Frame>
...
While you are waiting for the user to activate the device, begin polling the token URL to request an Access Token. Using the extracted polling interval (`interval`) from the previous step, you will need to `POST` to the [token URL](/docs/fr-ca/api/authentication#device-auth) sending along the `device_code`.
...
<tr>
<td>`device_code`</td>
<td>The `device_code` retrieved in the previous step of this tutorial.</td>
</tr>
<tr>
<td>`client_id`</td>
<td>Your application's Client ID. You can find this value in your <a href="https://manage.auth0.com/#/Applications/{yourClientId}/settings">Application Settings</a>.</td>
...
* a configured [Rule](/docs/fr-ca/rules) denied access

## User Authorization

The user will either scan the QR code, or else will open the activation page and enter the user code:

<Frame>![Enter User Code](https://cdn2.auth0.com/docs/1.14567.0/media/articles/flows/guides/device-auth/enter-user-code.png)</Frame>

A confirmation page will be shown to have the user confirm that this is the right device:

<Frame>![Confirm Device](https://cdn2.auth0.com/docs/1.14567.0/media/articles/flows/guides/device-auth/confirm-device.png)</Frame>
...
<Frame>![Authenticate User](https://cdn2.auth0.com/docs/1.14567.0/media/articles/flows/guides/device-auth/user-auth.png)</Frame>

Upon successful authentication and consent, the confirmation prompt will be shown:

<Frame>![User Confirmation](https://cdn2.auth0.com/docs/1.14567.0/media/articles/flows/guides/device-auth/user-confirmation.png)</Frame>
...
You should validate your tokens before saving them. To learn how, see [Validate Access Tokens](/docs/fr-ca/tokens/guides/validate-access-tokens) and [Validate ID Tokens](/docs/fr-ca/tokens/guides/validate-id-tokens).

</Warning>

[Access Tokens](/docs/fr-ca/tokens/concepts/access-token) are used to call the [Auth0 Authentication API's `/userinfo` endpoint](/docs/fr-ca/api/authentication#get-user-info) or another API. You will be able to use the Access Token to call `/userinfo` only if you included the `openid` scope. If you are calling your own API, the first thing your API will need to do is [verify the Access Token](/docs/fr-ca/tokens/guides/validate-access-tokens).

[ID Tokens](/docs/fr-ca/tokens/concepts/id-tokens) contain user information that must be [decoded and extracted](/docs/fr-ca/tokens/id-tokens#id-token-payload). The `id_token` will only be present in the response if you included the `openid` scope.

[Refresh Tokens](/docs/fr-ca/tokens/concepts/refresh-tokens) are used to obtain a new Access Token or ID Token after the previous one has expired. The `refresh_token` will only be present in the response if you included the `offline_access` scope and enabled **Allow Offline Access** for your API in the Dashboard.
...
You have already received a [Refresh Token](/docs/fr-ca/tokens/concepts/refresh-tokens) if you've been following this tutorial and completed the following:

* configured your API to allow offline access
* included the `offline_access` scope when you initiated the authentication request through the [authorize](/docs/fr-ca/api/authentication/reference#authorize-application) endpoint
...
You can use [Rules](/docs/fr-ca/rules) to detect whether the current transaction is using the Device Authorization Flow. To do so, check the `context` object's `protocol` property:
...
[Tenant logs](/docs/fr-ca/logs) are created for any interaction that takes place and can be used to troubleshoot issues.
...
* Have an [Auth0 application type](/docs/fr-ca/applications) of **Native**
* Have the [**Token Endpoint Authentication Method**](/docs/fr-ca/dashboard/reference/settings-application) set to **None**
* Be [OIDC-conformant](/docs/fr-ca/dashboard/reference/settings-application#oauth)
* Not be created through [Dynamic Client Registration](/docs/fr-ca/api-auth/dynamic-client-registration)

In addition, the Device Authorization Flow does not allow:

* [Social Connections](/docs/fr-ca/connections) using [Auth0 developer keys](/docs/fr-ca/connections/social/devkeys) unless you are using new [New Universal Login Experience](/docs/fr-ca/universal-login/new).
...
* [The OAuth 2.0 protocol](/docs/fr-ca/protocols/oauth2)
* [The OpenID Connect protocol](/docs/fr-ca/protocols/oidc)
* [Tokens](/docs/fr-ca/tokens)
* [Tenant Logs for Devices](/docs/fr-ca/logs)
...
[Edit on GitHub](https://github.com/auth0/docs/edit/master/fr-ca/articles/quickstart/native/device/01-login.md)