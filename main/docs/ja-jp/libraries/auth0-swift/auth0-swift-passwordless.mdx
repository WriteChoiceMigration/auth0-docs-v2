...
<td>必須</td>
<td>（文字列）使用するメールに応じて、<code>email</code>または<code>phoneNumber</code>が必要です（どちらか1つ）。</td>
</tr>
<tr>
...
<td>任意</td>
<td>（文字列）使用するパスワードレストランザクションのタイプ（<code>.Code</code>または<code>.iOSLink</code>）。デフォルトでは<code>.Code</code>に設定されています。</td>
</tr>
<tr>
<td><code>connection</code></td>
<td>任意</td>
<td>（文字列）パスワードレス認証に使用する接続の名前。デフォルトでは、SMSのオーバーロードでは<code>sms</code>、メールのオーバーロードでは<code>email</code>にそれぞれ設定されています。</td>
</tr>
</tbody>
</table>