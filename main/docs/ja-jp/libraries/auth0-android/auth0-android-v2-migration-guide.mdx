...
詳細については、「[OpenID Connectプロトコル](/docs/ja-jp/protocols/openid-connect-protocol)」を参照してください。

## PKCEを使用した認可コード

v2は[Proof Key for Code Exchange（PKCE）を使用した認可コード](/docs/ja-jp/flows/authorization-code-flow-with-proof-key-for-code-exchange-pkce)フローに対応しています。`WebAuthProvider.Builder`の`withResponseType(@ResponseType int type)`メソッドは削除されました。
