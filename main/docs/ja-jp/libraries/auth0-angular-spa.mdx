...
`logout`を使用して、ユーザーをログアウトさせます。<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Auth0 Dashboard</Tooltip>の**［Allowed Logout URLs（許可されているログアウトURL）］**に`returnTo`が指定されていることを確認してください。
