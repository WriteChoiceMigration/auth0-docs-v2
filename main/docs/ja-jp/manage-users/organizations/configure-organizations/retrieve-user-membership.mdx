...
`Get User Organizations`エンドポイントに対して`GET`呼び出しを行います。`USER_ID`と`MGMT_API_ACCESS_TOKEN`のプレースホルダーの値を、それぞれユーザーIDと<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Management API</Tooltip>のアクセストークンに置き換えてください。
...
| `MGMT_API_ACCESS_TOKEN` | スコープ`read:organizations`を持つ[Management APIのアクセストークン](/docs/tokens/management-api-access-tokens)。|
...
<td>Insufficient scope; expected any of: <code>read:organizations</code>.（スコープが不足しています。<code>read:organizations</code>が必要です。）</td>
