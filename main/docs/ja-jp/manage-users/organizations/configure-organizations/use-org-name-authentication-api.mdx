...
* [Organizations](/docs/manage-users/organizations/configure-organizations/create-organizations)は、2つの名前で構成できます。1つは、一意の論理識別子の役割を果たす必須の名前値で、もう1つは、任意で設定できる、わかりやすい表示名です。`org_name` パラメーターは、必須の名前値のみを受け入れ、表示名値には対応していません。
* この機能はテナントレベルで管理されます。特定のOrganizationに対して個別に有効化することはできません。

</Note>

この機能は、<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Auth0 Dashboard</Tooltip>または<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">Management API</Tooltip>で有効にすることができます。
