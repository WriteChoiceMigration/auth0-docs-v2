...
```
```java java.java
HttpResponse response = Unirest.delete("https://{yourDomain}/api/v2/organizations/ORG_ID/members")
.header("authorization", "Bearer MGMT_API_ACCESS_TOKEN")
.header("cache-control", "no-cache")
...
<td>スコープ<code>delete:organization_members</code>を持つ<a href="/docs/tokens/management-api-access-tokens">Management APIのアクセストークン</a>。</td>
</tr>
<tr>
...
<td>Insufficient scope; expected any of: <code>delete:organization_members</code>.（スコープが不足しています。<code>delete:organization_members</code>が必要です。）</td>
