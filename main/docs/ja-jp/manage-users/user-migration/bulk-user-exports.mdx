...
`POST /api/v2/jobs/users-exports`エンドポイントを使用して、[接続](/docs/identityproviders)に関連付けられているすべてのユーザー、またはテナント内のすべてのユーザーをエクスポートするジョブを作成できます。
...
有効な[Management APIアクセストークン](/docs/api/management/v2/tokens)も必要です。

## 要求本文を作成する

必要に応じて、<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Auth0 Dashboard</Tooltip>で`connection_id`とAuth0テナントドメイン名を見つけます。以下の要求本文を含む新しいテキストファイルを作成します。
...
ユーザーをエクスポートするジョブを作成したら、[Get a Jobエンドポイント](/docs/api/management/v2#!/Jobs/get_jobs_by_id)を使用してそのステータスを確認できます。
