...
'og:description': "ここでは、ユーザーアカウントを様々なIDプロバイダーからAuth0にリンクする方法について説明します。"
'og:image': "https://cdn2.auth0.com/docs/1.14567.0/img/share-image.png"
'twitter:title': "ユーザーアカウントのリンク"
'twitter:description': "ここでは、ユーザーアカウントを様々なIDプロバイダーからAuth0にリンクする方法について説明します。"
---
# ユーザーアカウントのリンク
...
メタデータは、アカウントのリンク中に自動でマージされません。マージする場合は、Manage API[`/users/patch_users_by_id`](/docs/api/v2#!/Users/patch_users_by_id)エンドポイントを使用して、手動で行う必要があります。
...
シングルページアプリケーションでユーザー主導のアカウントリンクを実装する方法について詳しくは、[「Client-Side Account Linking（クライアント側のアカウントリンク）」](/docs/users/references/link-accounts-client-side-scenario)をご一読ください。通常のウェブアプリケーションでユーザー主導のアカウントリンクを実装する方法について詳しくは、[「Server-Side Account Linking（サーバー側のアカウントリンク）」](/docs/users/references/link-accounts-server-side-scenario)をご一読ください。
...
また、[Account Link拡張機能](/docs/extensions/account-link)を使用しても、同じ結果を得ることができます。
