...
<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Auth0 Dashboard</Tooltip>の**［Settings（設定）］**タブ（[［Dashboard］>［Applications（アプリケーション）］>［API］](https://manage.auth0.com/#/apis)）を使って、認可されたアプリケーションから使用可能な登録済みのAPIを構成します。APIを構成するには、リストのAPIの横にある**［...］**をクリックして**［Settings（設定）］**を選択するか、API名をクリックします。APIの作成および登録方法については、「[APIを登録する](/docs/ja-jp/get-started/auth0-overview/set-up-apis)」をお読みください。

<Frame>![Dashboard - アプリケーション - APIリスト](//images.ctfassets.net/cdy7uua7fh8z/3rhmhghYZDSi6YWHRA5yMQ/2bafc57d32df6eb415b9e59b62f41531/API_List.png)</Frame>
...
これらのフィールドは、最初にAPIを登録したときに設定されますが、Auth0 <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">Management API</Tooltip>の場合は例外です。変更可能なのは**［Name（名前）］**のみです。

<Frame>![Dashboard - アプリケーション - APIの設定タブ - 一般設定](//images.ctfassets.net/cdy7uua7fh8z/QCaXd3FSeP6SYqLfEJWBr/f33c6d4d2852c04b65257b5804a6814b/API_general_settings.png)</Frame>
...
<Frame>![Dashboard - API - トークンの設定 - 有効期限](//images.ctfassets.net/cdy7uua7fh8z/2FFfqqmntXL9CnGrSxea9M/99e770190f064a72b4bc18c8cd867ce9/API_-_Access_Token_Settings.png)</Frame>

* **Maximum <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-2" tip="">Access Token</Tooltip> Lifetime (Seconds)（アクセストークンの最大ライフタイム（秒単位））**：アクセストークンの有効期限が切れるまでの時間（秒単位）です。デフォルトの値は、86,400秒（24時間）です。設定できる最大値は、2,592,000秒（30日間）です。
...
* **<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-3" tip="">JSON Web Token</Tooltip> (JWT) Profile（JSON Web Token（JWT）プロファイル）**：プロファイルは、APIに発行されるアクセストークンの形式を決定します。使用可能な値は`Auth0`と`RFC 9068`です。詳細については、「[アクセストークンのプロファイル](/docs/ja-jp/secure/tokens/access-tokens/access-token-profiles)」をお読みください。
* **JSON Web Token (JWT) <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-4" tip="">Signing Algorithm</Tooltip>（JSON Web Token （JWT）の署名アルゴリズム）**：トークンの署名に使用するアルゴリズムです。署名は、JWTの送信者が自称のとおりであることを検証し、メッセージが途中で変更されていないことを保証するために使用されます。使用可能な値は[`HS256`](https://en.wikipedia.org/wiki/Symmetric-key_algorithm)と[`RS256`](https://en.wikipedia.org/wiki/Public-key_cryptography)です。`RS256`（推奨）を選択すると、トークンがテナントの秘密鍵で署名されます。この値は、APIの作成時に設定され、以降変更することはできません。署名アルゴリズムとAuth0での役割については、「[署名アルゴリズム](/docs/ja-jp/get-started/applications/signing-algorithms)」をお読みください。

  <Note>

  署名はJWTの一部です。JWTの構造に慣れていない場合は、「[JSON Web Tokenの構造](/docs/secure/tokens/json-web-tokens/json-web-token-structure)」を参照してください。
...
<Frame>![Auth0 Dashboard - APIの設定 - RBACトグル](//images.ctfassets.net/cdy7uua7fh8z/65tKb6aj0ktc2qXLUVlV3e/ee1e226b004fb56f88143ead67750fbf/API_-_RBAC_Settings.png)</Frame>
...
<Frame>![Dashboard - アプリケーション - APIの設定タブ - アクセスの設定](//images.ctfassets.net/cdy7uua7fh8z/JNlWACqoyfmpE89JwWLKy/c1e88ad09e1d33d5689eb426d96bbe43/2025-02-25_14-53-56.png)</Frame>
...
<Frame>![Dashboard - APIの権限を追加する - APIの権限の定義画面](//images.ctfassets.net/cdy7uua7fh8z/32Pb185OFs2mC6z2fmunEw/6b337d23304ea568dfcf8d9156b859b8/2025-02-27_10-17-20.png)</Frame>
...
<Frame>![Dashboard - アプリケーション - API - マシンツーマシンアプリケーション](//images.ctfassets.net/cdy7uua7fh8z/7lIxRDYBUf0CxzZkXFZFyI/2db7526b16cb6c427057e8cdd6ea1ba1/Travel0_API_m2m_tab.png)</Frame>
...
<Frame>![Dashboard - アプリケーション - APIのテストタブ](//images.ctfassets.net/cdy7uua7fh8z/iVNH8BQ3GBp90CSJTbArs/53bc8f41e828a4d27bfd792bb1f21454/Travel0_API_-_Mgmt_API_Tab.png)</Frame>

## Management API Explorer

Auth0 Management APIの**［Settings（設定）］**を表示すると、**［API Explorer］**と呼ばれる追加のタブが表示されます。

<Frame>![Dashboard - アプリケーション - API - Management API Explorer](//images.ctfassets.net/cdy7uua7fh8z/1OSC4OQZkMBQOq9D0PFuHv/82b4600bc5a72b96627e471cb1613c53/Auth0_Management_API_settings.png)</Frame>
