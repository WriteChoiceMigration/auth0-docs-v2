...
* `favorite_color`プロパティも`user_metadata`プロパティも存在しません。これは、<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">OpenID</Tooltip> Connect (OIDC)が`favorite_color`または`user_metadata`を表す標準クレームを定義しないためです。
