...
アプリケーションにGuardian SDKをインストールする方法については、「[Auth0 Guardian](/docs/ja-jp/secure/multi-factor-authentication/auth0-guardian#guardian-sdks)」と、お使いのモバイルデバイスプラットフォームに関連するセクションをお読みください。
...
2.［**Push Notification using Auth0 Guardian（Auth0 Guardianを使ったプッシュ通知）**］を有効にします。このために<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">MFA</Tooltip>の構成設定が必要になる場合があります。詳細については、「[MFAにプッシュ通知を構成する](/docs/ja-jp/secure/multi-factor-authentication/multi-factor-authentication-factors/configure-push-notifications-for-mfa)」を参照してください。
