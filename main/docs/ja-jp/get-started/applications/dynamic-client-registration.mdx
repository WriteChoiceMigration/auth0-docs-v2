...
または、<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Management API</Tooltip> [`/Tenant/patch_settings`](/docs/ja-jp/api/management/v2#!/Tenants/patch_settings)エンドポイントを使用してこのフラグを更新することができます。
...
* **client_secret**：英数字の64ビットのクライアントシークレット。Authentication API [`/token`](/docs/ja-jp/api/authentication#get-token)への認証と、IDトークンの検証にアプリケーションで使用される値です。
