...
[`/Clients/delete_clients_by_id`](/docs/ja-jp/api/management/v2#!/Clients/delete_clients_by_id)エンドポイントに`DELETE`呼び出しを行います。必ず、`YOUR_CLIENT_ID`と`MGMT_API_ACCESS_TOKEN`のプレースホルダーの値を、それぞれご自身のクライアントIDと<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">Management API</Tooltip>のアクセストークンに置き換えてください。
...
<td>`delete:clients`スコープのある[Management APIの<a href="/docs/ja-jp/api/management/v2/tokens">アクセストークン</a>。</td>
</tr>
</tbody>
</table>
