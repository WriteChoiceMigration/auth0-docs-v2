...
アプリケーションは、<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Auth0 Dashboard</Tooltip>または管理APIを使って削除できます。この操作は、一度確定すると元に戻せません。
...
[`/Clients/delete_clients_by_id`](/docs/api/management/v2#!/Clients/delete_clients_by_id)エンドポイントに`DELETE`呼び出しを行います。必ず、`YOUR_CLIENT_ID`と`MGMT_API_ACCESS_TOKEN`のプレースホルダーの値を、それぞれご自身のクライアントIDと<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">Management API</Tooltip>のアクセストークンに置き換えてください。
...
<td><code>delete:clients</code>スコープのある[Management APIの<a href="https://auth0.com/docs/ja-jp/api/management/v2/tokens">アクセストークン</a>。</td>
</tr>
</tbody>
</table>
