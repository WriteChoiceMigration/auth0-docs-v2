...
* テナント名には、Auth0ドメインにおけるロールが含まれます。名前を決める前に、「[テナントの特性](/docs/ja-jp/get-started/auth0-overview/create-tenants#create-a-tenant-and-domain)」をお読みください。
...
* 「[バックエンドサービスおよびAPIライブラリー](/docs/ja-jp/libraries#backend)」をお読みのうえ、クイックスタートをお試しください。
...
[［Authentication（認証）］](/docs/ja-jp/get-started/onboarding/self-service-m2m#authentication)セクションで、M2M認証用のAPI設定を構成します。
...
5. **［Update（更新）］**を選択します。
   ...
   <Frame>![［Dashboard］ > ［Applications（アプリケーション）］ > ［APIs］](/images/ja-jp/cdy7uua7fh8z/6L2R46XVdYw1kifRfMifRz/364292a89a04a152938e6901072ea662/Timesheet_API.png)</Frame>
...
トークンベースの認証では、非対話型クライアントは、[Authentication APIトークンエンドポイント](/docs/ja-jp/customize/actions/flows-and-triggers/machine-to-machine-flow)への呼び出しで`client_id`と`client_secret`を提供し、[アクセストークン](/docs/ja-jp/secure/tokens/access-tokens)を取得します。このアクセストークンにより、保護されたAPIへのアクセスが許可されます。
...
* 独自の証明書を管理するか、Auth0管理の証明書を使用するかを決定します。証明書の詳細については、「[証明書管理のオプション](/docs/ja-jp/customize/custom-domains#certificate-management-options)」をお読みください。
...
2. 「[カスタムドメインを使用したAPI構成](/docs/ja-jp/customize/custom-domains/configure-features-to-use-custom-domains#apis)」を確認してください。カスタムドメインを組み込むには、API設定を調整する必要がある場合があります。
...
<Frame>![Auth0 Dashboard > Readiness Checklist](/images/ja-jp/cdy7uua7fh8z/7iu1CzH0NgaXJbLBNWXZli/ea20f97de93711c0c995ea9fedcf20fa/Readiness_Checks_-_Japanese.png)</Frame>
