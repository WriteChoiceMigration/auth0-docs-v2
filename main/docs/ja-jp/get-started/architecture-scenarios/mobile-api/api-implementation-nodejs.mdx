...
* アクセストークンを取得する。詳しい取得方法については、「[アクセストークンを取得する](/docs/architecture-scenarios/application/server-api#get-an-access-token)」を参照してください。
...
3. Actionsコードエディターを見つけて、次のJavaScriptコードをコピーし、**［Save Draft（下書きを保存）］**を選択して変更を保存します。
   ...
   `namespace`（名前空間）を使うと、クレームの名前が標準のOIDCクレームや内部サービスと衝突しない、一意のものになります。名前空間のあるクレームと名前空間のないクレームの制約やガイドラインについては、「[カスタムクレームを作成する](/docs/secure/tokens/json-web-tokens/create-custom-claims)」をお読みください。
