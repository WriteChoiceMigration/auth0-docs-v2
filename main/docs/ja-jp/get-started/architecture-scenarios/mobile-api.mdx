...
* APIは、APIへの呼び出し時にHTTP認可ヘッダーに有効な<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">アクセストークン</Tooltip>が渡されることを確認することで保護されます（[「APIを実装する」](/docs/ja-jp/get-started/architecture-scenarios/mobile-api/part-3#secure-the-endpoints)を参照）
...
* モバイルアプリは、APIへの呼び出し時にHTTP認可ヘッダーにアクセストークンを提供します（[「APIを呼び出す」](/docs/get-started/architecture-scenarios/mobile-api/part-3#call-the-api)参照）
