...
[サーバー＋APIアーキテクチャーシナリオ](/docs/ja-jp/architecture-scenarios/application/server-api)の一環として、Node.jsでタイムシートAPIを実装する方法について説明します。実装したソリューションについての情報は、シナリオを参照してください。
...
* アクセストークンを取得する。詳しい取得方法については、以下をご覧ください：[アクセストークンを取得する](/docs/ja-jp/architecture-scenarios/application/server-api#get-an-access-token)
