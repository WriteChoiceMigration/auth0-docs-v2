...
Auth0に<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Management API</Tooltip> v2トークンを求めるには、前提条件のステップで作成したM2Mアプリケーションの資格情報を使用して、`POST`操作を`https://{yourDomain}/oauth/token`エンドポイントに実行します。
...
<td>実行したい<a href="/docs/protocols/oauth2#grant-types">OAuth 2.0フロー</a>を示します。マシンツーマシン通信については、<code>client_credentials</code>の値を使用します。</td>
</tr>
<tr>
...
<td>これは<code>Auth0 Management API</code>の <strong>識別子</strong> フィールドの値です。<a href="https://manage.auth0.com/#/apis">APIの設定タブ</a>で見つけることができます。</td>
</tr>
</tbody>
</table>
...
たとえば、[すべてのアプリケーションを取得](/docs/api/management/v2#!/Clients/get_clients)するには、以下を使用します。
...
このPythonスクリプトはManagement API v2のアクセストークンを取得し、[すべてのアプリケーション取得](/docs/api/management/v2#!/Clients/get_clients)エンドポイントの呼び出しに使用して、コンソールに応答を出力します。
...
* `CLIENT_ID`：作成したM2Mアプリケーションの**<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">Client ID</Tooltip>（クライアントID）**。
* `CLIENT_SECRET`：作成したM2Mアプリケーションの**<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-2" tip="">Client Secret</Tooltip>（クライアントシークレット）**。
