...
Duo Securityを構成するには、<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Auth0 Dashboard</Tooltip>でアプリケーションにDuoの資格情報を渡す必要があります。
...
Duoには、「Remember Me（自動ログイン）」の動作オプションはありません。初回のログイン後にユーザーを記憶しておく、30日間の<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">MFA</Tooltip>セッションがハードコードされています。
...
exports.onExecutePostLogin = async (event, api) => {
	...
	if (CLIENTS_WITH_MFA.includes(event.client.client_id)) {
		...
		// See /docs/multifactor-authentication/custom#change-the-frequency-of-authentication-requests for details
