...
* この機能を使用すると、MFA選択フローをカスタマイズすることができます。MFA登録フローをカスタマイズしたい場合は、「[ユニバーサルログインでのMFA登録をカスタマイズする](/docs/secure/multi-factor-authentication/customize-mfa/customize-mfa-enrollments-universal-login)」を参照してください。
* [ユニバーサルログイン](/docs/authenticate/login/auth0-universal-login/universal-login-vs-classic-login/universal-experience)を使用している場合、MFAフローはActionsを使わないとカスタマイズできません。
...
<Frame>![［Auth0 Dashboard］>［Security（セキュリティ）］>［MFA（多要素認証）］>［Additional Settings（追加設定）］](//images.ctfassets.net/cdy7uua7fh8z/2hv0ELTkkka3t230SXfxw/3df4a2e61198f7c2cfbf6864df592b39/MFA_-_Actions_toggle_-_Japanese.png)</Frame>
...
ユーザーがサインアップまたはテナントで作成された後、<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-2" tip="">Management API</Tooltip>の[authentication-methods](/docs/api/management/v2/users/post-authentication-methods)エンドポイントを使用して登録を作成する必要があります。または、Auth0 Dashboardのプロファイルページから直接ユーザーの登録を管理することができます。
...
3. ユーザーはアクションで参照された要素に登録されている必要があります。
   ...
   * エラーが発生した場合は、対象ユーザーの詳細を確認し、適切なファクターに登録されていることを確認してください。[**［Auth0 Dashboard］>［ユーザー管理］>［ユーザー］**](https://manage.auth0.com/#/users)に移動し、リストからユーザーの名前を選択します。
     ...
     + または、**［Raw JSON（生JSON）］タブ**でユーザーの登録状況を確認してください。この情報は、[Management API](/docs/api/management/v2/users/get-authentication-methods)を使用することでも取得できます。ただし、APIには、メール認証リンクを介して設定されたメール要素のような自動登録された認証手段は表示されないことに注意が必要です。
