...
4. **［Application Restrictions（アプリケーションの制限）］**で**［IP addresses (web servers, cron jobs, etc.)（IPアドレス（ウェブサーバー、cron jobsなど）］**を選択し、さらに[［Auth0 IP Addresses for Allow Lists（許可リストのAuth0 IPアドレス）］](/docs/ja-jp/secure/security-guidance/data-security/allowlist)を入力します。
