...
CSRF攻撃では、偽の要求に対する応答を知る手段が攻撃者にないため、ユーザーデータの取得ではなく、状態変更要求に的を絞って攻撃し、操作を仕掛けようとします。基本的に、状態パラメーターは<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">nonce</Tooltip>となり、要求と認証からの応答を関係付けるのに使用されます。

最新のOIDCと<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">OAuth</Tooltip> SDKでは、シングルページアプリケーションにAuth0.jsが含まれており、状態の生成と検証が自動的に処理されます。
