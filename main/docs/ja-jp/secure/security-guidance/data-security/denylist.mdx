...
プライバシー上の理由により Auth0 データベースに保存すべきでないユーザーフィールドがある場合は、それらを拒否リストに追加できます。拒否リストに属性を追加するには、<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Management API</Tooltip>の更新接続エンドポイントに対して`PATCH`呼び出しを行います。

1. `/patch_connections_by_id`[エンドポイント](/docs/api/management/v2#!/Connections/patch_connections_by_id)にアクセスするための有効なアクセストークンを取得します。トークンには、`update:connections`スコープが含まれている必要があります。詳細については、[［Management API Access Tokens（Management APIのアクセストークン）］](/docs/ja-jp/secure/tokens/access-tokens/management-api-access-tokens)を参照してください。
...
```bash curl.sh
   ...
   ```
   ```java java.java
   HttpResponse response = Unirest.patch("https://{yourDomain}/api/v2/connections/YOUR_CONNECTION_ID")
   .header("authorization", "Bearer YOUR_TOKEN")
   .header("content-type", "application/json")
