...
3. クライアント側は Auth0 とのコード交換を完了し、ユーザーの`id_token`と`access_token`を取得します。これらはメモリに保存されます。
   ...
   <Frame>![トークン保管のベストプラクティスのインメモリストレージ図](//images.ctfassets.net/cdy7uua7fh8z/6a4aA0TH8PJQpvhkLaGSIp/107dc0bf68b1829640a75bf004393623/in-memory-token-storage.png)</Frame>
...
* [通常のWebアプリのクイックスタート](/docs/quickstart/webapp)
...
* [ネイティブ/モバイルアプリのQuickstarts](/docs/quickstart/native)
...
OAuth2の仕様に準拠して、ブラウザーが/tokenエンドポイントからリフレッシュトークンを要求したとき、Auth0は、そのクライアントに対して[リフレッシュトークンのローテーション](/docs/secure/tokens/refresh-tokens/refresh-token-rotation)が有効になっている場合のみリフレッシュトークンを返します。
