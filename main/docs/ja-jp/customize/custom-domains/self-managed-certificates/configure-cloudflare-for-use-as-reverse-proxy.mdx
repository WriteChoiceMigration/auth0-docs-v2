...
1. [カスタムドメインを構成して自己管理証明書を使って検証する](/docs/ja-jp/customize/custom-domains/self-managed-certificates)作業がまだ済んでいない場合は、ここで行います。**［Origin Domain Name（オリジンのドメイン名）］**と**cname-api-key**の値が後で必要になるのでメモしておきます。
...
以下のペイロードを本文に含むAuth0 <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Management API</Tooltip>の[カスタムドメイン構成の更新](https://auth0.com/docs/api/management/v2#!/Custom_Domains/patch_custom_domains_by_id)エンドポイントを呼び出します。
