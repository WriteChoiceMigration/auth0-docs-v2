...
**Scenario: Custom phone provider configured using Terraform is not taking effect, and the <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Auth0 Dashboard</Tooltip> displays a different Action.**
...
To reset the custom phone provider, navigate to [Auth0 Dashboard > Branding](/docs/ja-jp/$manage_url#/branding) and select **Phone Provider**. Then, select the **Reset** button to restore your custom provider to its default