...
* <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">OpenID</Tooltip>の<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">JSON Web Token</Tooltip>（JWT）を渡して、サービスへIDを送る
* トークンを検証する
...
ユーザーのメールアドレスをJWTに追加する方法の1つが、[ルール](/docs/rules)です。この方法は、値がユーザー認証用のJWTで常に利用可能なことを確実にしたい場合には適切です。
...
トークン検証の詳細については、「[Auth0が対応しているIDプロトコル](/docs/ja-jp/authenticate/protocols)」をご覧ください。

秘密鍵で`auth0-variables.js`を更新します。秘密鍵は、<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-2" tip="">Auth0 Dashboard</Tooltip>にある［Application（アプリケーション）］の［Settings（設定）］タブに表示されます。
...
* APIへのアクセスをユーザーベースと結び付けて、<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-3" tip="">SAML</Tooltip> IDプロバイダーとIAMを統合する
