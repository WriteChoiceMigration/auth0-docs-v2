...
* 検証の実施や<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">MFA</Tooltip>のトリガーを行うアクションが、意図せずまたは悪意のある方法でバイパスされることがないようにします。
...
* シークレットや機微コードアーティファクトをアクションコードの一部としてテキスト形式で記述しないでください。代わりに、[シークレットマネージャー](/docs/ja-jp/customize/actions/write-your-first-action#add-a-secret)を使用するか、独自のマネージャーをアクションコード内で統合して利用します。
