...
Post-Loginアクションは既存のルールの後に実行されるため、Dashboardでルールを一度に1つずつ変換することも、 <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Management API</Tooltip>を使用して一度にすべてを変換することもできます。
...
あらゆるリダイレクトをアクションで適切に行う方法について、このガイドで説明することはできません。詳細については、「[アクションを使ったリダイレクト](/docs/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/redirect-with-actions)」をお読みください。
