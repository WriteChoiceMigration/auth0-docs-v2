...
<Frame>![認証フロー図のルール](//images.ctfassets.net/cdy7uua7fh8z/2gtBtkPChIyguA24x6enx2/f852c2589c1cc8f6d0801be70866c67a/flow.png)</Frame>
...
[ルール作成エンドポイント](/docs/api/management/v2#!/Rules/post_rules)に`POST`呼び出しを行います。`MGMT_API_ACCESS_TOKEN`、`RULE_NAME`、`RULE_SCRIPT`、`RULE_ORDER`、および`RULE_ENABLED`のプレースホルダーの値をそれぞれ、実際の<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">Management API</Tooltip>のアクセストークン、ルール名、ルールスクリプト、ルールの順序、ルールの有効化の値に置き換えてください。
...
<td><code>create:rules</code>スコープを持つManagement APIのアクセストークン。</td>
</tr>
<tr>
...
<tr>
<td><code>RULE_ORDER</code>（任意）</td>
<td>他のルールとの関係でルールが実行される順番を示す整数。より小さい番号のルールは、大きな番号のルールより先に実行されます。順番の番号が提供されなかった場合、ルールは最後に実行されます。</td>
</tr>
<tr>
<td><code>RULE_ENABLED</code>（任意）</td>
<td>ルールが有効（<code>true</code>）か無効（<code>false</code>）かを示すブール値。</td>
</tr>
</tbody>
</table>
