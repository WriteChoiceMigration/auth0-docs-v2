...
'og:title': "Login"
'og:description': "This quickstart demonstrates how to add user login to an Android application using Auth0."
'og:image': "https://cdn2.auth0.com/docs/1.14567.0/media/platforms/android.png"
'twitter:title': "Login"
'twitter:description': "This quickstart demonstrates how to add user login to an Android application using Auth0."
---
...
We've used a value of `demo` for `auth0Scheme` here, so that a custom URL scheme can be used for the URL that Auth0 redirects to after login. The alternative is `https` if you want to use [Android App Links](/docs/ja-jp/applications/enable-android-app-links). You can read more about setting this value in the [Auth0.Android SDK readme](https://github.com/auth0/Auth0.Android#a-note-about-app-deep-linking).
...
[Universal Login](/docs/ja-jp/hosted-pages/login) is the easiest way to set up authentication in your application. We recommend using it for the best experience, best security and the fullest array of features.
...
There are many options to customize the authentication with the `WebAuthProvider` builder. You can read about them in the [Auth0 SDK for Android documentation](/docs/ja-jp/libraries/auth0-android).
...
Use the `AuthenticationAPIClient` class to [retrieve the user's profile from Auth0](/docs/ja-jp/users/user-profiles#user-profile-management-api-access). This requires:
...
[Edit on GitHub](https://github.com/auth0/docs/edit/master/ja-jp/articles/quickstart/native/android/00-login.md)