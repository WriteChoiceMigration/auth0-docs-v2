...
[##### Expoアプリケーションにログインを追加する](#Expoアプリケーションにログインを追加する)
...
5. [Expo](/docs/ja-jp/quickstart/native/react-native-expo/interactive)
...
## Auth0構成プラグインをセットアップする](#auth0構成プラグインをセットアップする)
...
## ネイティブソースコードを生成する](#ネイティブソースコードを生成する)
...
`Auth0Provider`コンポーネントが構成されました。アプリケーションを実行して次の点を確認します：
...
`clearSession`を呼び出すログアウトボタンを追加して、Auth0のログアウトエンドポイントにリダイレクトされ、再び戻されることを確認してください。アプリケーションにはログインされていないはずです。
...
`useAuth0`フックは、認証済みユーザーの情報を含む`user`オブジェクトを公開します。これを使用すれば、認証済みユーザーについてデコードされたユーザープロファイル情報に、[IDトークン](https://auth0.com/docs/secure/tokens/id-tokens)からアクセスできます。
...
[Edit on GitHub](https://github.com/auth0/docs/edit/master/ja-jp/articles/quickstart/native/react-native-expo/interactive.md)
