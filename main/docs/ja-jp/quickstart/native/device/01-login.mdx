...
This tutorial will help you call your own API from an input-constrained device using the Device Authorization Flow. If you want to learn how the flow works and why you should use it, see [Device Authorization Flow](/docs/ja-jp/flows/concepts/device-auth).
...
* [Register the Application with Auth0](/docs/ja-jp/dashboard/guides/applications/register-app-native).
  ...
  + Make sure the Application's **[Grant Types](/docs/ja-jp/dashboard/guides/applications/update-grant-types)** include **Device Code**. This is also in the [Dashboard](https://manage.auth0.com/#), under **Application Settings > Advanced > Grant Types**.
  + If you want your Application to be able to use [Refresh Tokens](/docs/ja-jp/tokens/concepts/refresh-tokens), make sure the Application's **[Grant Types](/docs/ja-jp/dashboard/guides/applications/update-grant-types)** include **Refresh Token**.
* Set up and enable at least one connection for the Application: [Database connections](/docs/ja-jp/dashboard/guides/connections/set-up-connections-database), [Social connections](/docs/ja-jp/dashboard/guides/connections/set-up-connections-social)
* [Register your API with Auth0](/docs/ja-jp/architecture-scenarios/mobile-api/part-2#create-the-api)

  + If you want your API to receive [Refresh Tokens](/docs/ja-jp/tokens/concepts/refresh-tokens) to allow it to obtain new tokens when the previous ones expire, enable **Allow Offline Access**.
* [Configure Device User Code Settings](/docs/ja-jp/dashboard/guides/tenants/configure-device-user-code-settings) to define the character set, format, and length of your randomly-generated user code.
...
To get the device code, your app must request a code from the [device code URL](/docs/ja-jp/api/authentication#get-device-code), including the Client ID.
...
<tr>
<td>`scope`</td>
<td>The <a href="/docs/ja-jp/scopes">scopes</a> for which you want to request authorization. These must be separated by a space. You can request any of the <a href="https://openid.net/specs/openid-connect-core-1_0.html#StandardClaims">standard OIDC scopes</a> about users, such as `profile` and `email`, <a href="/docs/ja-jp/tokens/concepts/jwt-claims#custom-claims">custom claims</a> conforming to a <a href="/docs/ja-jp/tokens/guides/create-namespaced-custom-claims">namespaced format</a>, or any <a href="/docs/ja-jp/scopes/current/api-scopes">scopes supported by the target API</a> (e.g., `read:contacts`). Include `openid` to get an ID Token or to be able to use the <a href="/docs/ja-jp/api/authentication#user-profile">/userinfo endpoint</a> to retrieve profile information for the user. Include `offline_access` to get a Refresh Token (make sure that the <strong>Allow Offline Access</strong> field is enabled in the <a href="https://manage.auth0.com/#/apis">API Settings</a>). Note that this must be URL encoded.</td>
</tr>
<tr>
<td>`audience`</td>
...
You can [configure the character set, format, and length of your randomly-generated user code](/docs/ja-jp/dashboard/guides/tenants/configure-device-user-code-settings) in your tenant settings.
...
While you are waiting for the user to activate the device, begin polling the token URL to request an Access Token. Using the extracted polling interval (`interval`) from the previous step, you will need to `POST` to the [token URL](/docs/ja-jp/api/authentication#device-auth) sending along the `device_code`.
...
* a configured [Rule](/docs/ja-jp/rules) denied access
...
You should validate your tokens before saving them. To learn how, see [Validate Access Tokens](/docs/ja-jp/tokens/guides/validate-access-tokens) and [Validate ID Tokens](/docs/ja-jp/tokens/guides/validate-id-tokens).

</Warning>

[Access Tokens](/docs/ja-jp/tokens/concepts/access-token) are used to call the [Auth0 Authentication API's `/userinfo` endpoint](/docs/ja-jp/api/authentication#get-user-info) or another API. You will be able to use the Access Token to call `/userinfo` only if you included the `openid` scope. If you are calling your own API, the first thing your API will need to do is [verify the Access Token](/docs/ja-jp/tokens/guides/validate-access-tokens).

[ID Tokens](/docs/ja-jp/tokens/concepts/id-tokens) contain user information that must be [decoded and extracted](/docs/ja-jp/tokens/id-tokens#id-token-payload). The `id_token` will only be present in the response if you included the `openid` scope.

[Refresh Tokens](/docs/ja-jp/tokens/concepts/refresh-tokens) are used to obtain a new Access Token or ID Token after the previous one has expired. The `refresh_token` will only be present in the response if you included the `offline_access` scope and enabled **Allow Offline Access** for your API in the Dashboard.
...
You have already received a [Refresh Token](/docs/ja-jp/tokens/concepts/refresh-tokens) if you've been following this tutorial and completed the following:

* configured your API to allow offline access
* included the `offline_access` scope when you initiated the authentication request through the [authorize](/docs/ja-jp/api/authentication/reference#authorize-application) endpoint
...
You can use [Rules](/docs/ja-jp/rules) to detect whether the current transaction is using the Device Authorization Flow. To do so, check the `context` object's `protocol` property:
...
[Tenant logs](/docs/ja-jp/logs) are created for any interaction that takes place and can be used to troubleshoot issues.
...
* Have an [Auth0 application type](/docs/ja-jp/applications) of **Native**
* Have the [**Token Endpoint Authentication Method**](/docs/ja-jp/dashboard/reference/settings-application) set to **None**
* Be [OIDC-conformant](/docs/ja-jp/dashboard/reference/settings-application#oauth)
* Not be created through [Dynamic Client Registration](/docs/ja-jp/api-auth/dynamic-client-registration)

In addition, the Device Authorization Flow does not allow:

* [Social Connections](/docs/ja-jp/connections) using [Auth0 developer keys](/docs/ja-jp/connections/social/devkeys) unless you are using new [New Universal Login Experience](/docs/ja-jp/universal-login/new).
...
* [The OAuth 2.0 protocol](/docs/ja-jp/protocols/oauth2)
* [The OpenID Connect protocol](/docs/ja-jp/protocols/oidc)
* [Tokens](/docs/ja-jp/tokens)
* [Tenant Logs for Devices](/docs/ja-jp/logs)
...
[Edit on GitHub](https://github.com/auth0/docs/edit/master/ja-jp/articles/quickstart/native/device/01-login.md)