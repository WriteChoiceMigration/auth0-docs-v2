...
This tutorial will help you call your own API from an input-constrained device using the Device Authorization Flow. If you want to learn how the flow works and why you should use it, see [Device Authorization Flow](/docs/flows/concepts/device-auth).
...
* [Register the Application with Auth0](/docs/dashboard/guides/applications/register-app-native).
  ...
  + Make sure the Application's **[Grant Types](/docs/dashboard/guides/applications/update-grant-types)** include **Device Code**. This is also in the [Dashboard](https://manage.auth0.com/#), under **Application Settings > Advanced > Grant Types**.
  + If you want your Application to be able to use [Refresh Tokens](/docs/tokens/concepts/refresh-tokens), make sure the Application's **[Grant Types](/docs/dashboard/guides/applications/update-grant-types)** include **Refresh Token**.
* Set up and enable at least one connection for the Application: [Database connections](/docs/dashboard/guides/connections/set-up-connections-database), [Social connections](/docs/dashboard/guides/connections/set-up-connections-social)
* [Register your API with Auth0](/docs/architecture-scenarios/mobile-api/part-2#create-the-api)

  + If you want your API to receive [Refresh Tokens](/docs/tokens/concepts/refresh-tokens) to allow it to obtain new tokens when the previous ones expire, enable **Allow Offline Access**.
* [Configure Device User Code Settings](/docs/dashboard/guides/tenants/configure-device-user-code-settings) to define the character set, format, and length of your randomly-generated user code.
...
To get the device code, your app must request a code from the [device code URL](/docs/api/authentication#get-device-code), including the Client ID.
...
<td>The <a href="/docs/scopes">scopes</a> for which you want to request authorization. These must be separated by a space. You can request any of the <a href="https://openid.net/specs/openid-connect-core-1_0.html#StandardClaims">standard OIDC scopes</a> about users, such as <code>profile</code> and <code>email</code>, <a href="/docs/tokens/concepts/jwt-claims#custom-claims">custom claims</a> conforming to a <a href="/docs/tokens/guides/create-namespaced-custom-claims">namespaced format</a>, or any <a href="/docs/scopes/current/api-scopes">scopes supported by the target API</a> (e.g., <code>read:contacts</code>). Include <code>openid</code> to get an ID Token or to be able to use the <a href="/docs/api/authentication#user-profile">/userinfo endpoint</a> to retrieve profile information for the user. Include <code>offline_access</code> to get a Refresh Token (make sure that the <strong>Allow Offline Access</strong> field is enabled in the <a href="https://manage.auth0.com/#/apis">API Settings</a>). Note that this must be URL encoded.</td>
</tr>
<tr>
...
You can [configure the character set, format, and length of your randomly-generated user code](/docs/dashboard/guides/tenants/configure-device-user-code-settings) in your tenant settings.
...
While you are waiting for the user to activate the device, begin polling the token URL to request an Access Token. Using the extracted polling interval (`interval`) from the previous step, you will need to `POST` to the [token URL](/docs/api/authentication#device-auth) sending along the `device_code`.
...
* a configured [Rule](/docs/rules) denied access
...
You should validate your tokens before saving them. To learn how, see [Validate Access Tokens](/docs/tokens/guides/validate-access-tokens) and [Validate ID Tokens](/docs/tokens/guides/validate-id-tokens).

</Warning>

[Access Tokens](/docs/tokens/concepts/access-token) are used to call the [Auth0 Authentication API's `/userinfo` endpoint](/docs/api/authentication#get-user-info) or another API. You will be able to use the Access Token to call `/userinfo` only if you included the `openid` scope. If you are calling your own API, the first thing your API will need to do is [verify the Access Token](/docs/tokens/guides/validate-access-tokens).

[ID Tokens](/docs/tokens/concepts/id-tokens) contain user information that must be [decoded and extracted](/docs/tokens/id-tokens#id-token-payload). The `id_token` will only be present in the response if you included the `openid` scope.

[Refresh Tokens](/docs/tokens/concepts/refresh-tokens) are used to obtain a new Access Token or ID Token after the previous one has expired. The `refresh_token` will only be present in the response if you included the `offline_access` scope and enabled **Allow Offline Access** for your API in the Dashboard.
...
You have already received a [Refresh Token](/docs/tokens/concepts/refresh-tokens) if you've been following this tutorial and completed the following:

* configured your API to allow offline access
* included the `offline_access` scope when you initiated the authentication request through the [authorize](/docs/api/authentication/reference#authorize-application) endpoint
...
You can use [Rules](/docs/rules) to detect whether the current transaction is using the Device Authorization Flow. To do so, check the `context` object's `protocol` property:
...
[Tenant logs](/docs/logs) are created for any interaction that takes place and can be used to troubleshoot issues.
...
* Have an [Auth0 application type](/docs/applications) of **Native**
* Have the [**Token Endpoint Authentication Method**](/docs/dashboard/reference/settings-application) set to **None**
* Be [OIDC-conformant](/docs/dashboard/reference/settings-application#oauth)
* Not be created through [Dynamic Client Registration](/docs/api-auth/dynamic-client-registration)

In addition, the Device Authorization Flow does not allow:

* [Social Connections](/docs/connections) using [Auth0 developer keys](/docs/connections/social/devkeys) unless you are using new [New Universal Login Experience](/docs/universal-login/new).
...
* [The OAuth 2.0 protocol](/docs/protocols/oauth2)
* [The OpenID Connect protocol](/docs/protocols/oidc)
* [Tokens](/docs/tokens)
* [Tenant Logs for Devices](/docs/logs)
