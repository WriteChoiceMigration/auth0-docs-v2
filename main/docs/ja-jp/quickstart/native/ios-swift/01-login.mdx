...
'og:title': "Login"
'og:description': "This guide demonstrates how to integrate Auth0 with any new or existing iOS / macOS app using the Auth0.swift SDK."
'og:image': "https://cdn2.auth0.com/docs/1.14567.0/media/platforms/ios.png"
'twitter:title': "Login"
'twitter:description': "This guide demonstrates how to integrate Auth0 with any new or existing iOS / macOS app using the Auth0.swift SDK."
---
...
You will need a **Native** Auth0 application. If you don’t have a Native Auth0 application already, [create one](/docs/ja-jp/get-started/auth0-overview/create-applications/native-apps) before continuing. Avoid using other application types, as they have different configurations and may cause errors.
...
The Auth0.swift SDK needs the **Client ID** and **domain** of the Auth0 application to communicate with Auth0. You can find these details in the [settings page](https://manage.auth0.com/#/applications/{yourClientId}/settings) of your Auth0 application. If you are using a [custom domain](/docs/ja-jp/customize/custom-domains), use the value of your custom domain instead of the value from the settings page.
...
Then, present the [Universal Login](/docs/ja-jp/authenticate/login/auth0-universal-login) page in the action of your **Login** button.
...
Now that you can log in to your app, you need a way to [log out](/docs/ja-jp/authenticate/login/logout). In the action of your **Logout** button, call the `clearSession()` method to clear the Universal Login session cookie.
...
The `Credentials` instance you obtained after logging in includes an [ID Token](/docs/ja-jp/secure/tokens/id-tokens). The ID Token contains the profile information associated with the logged-in user, such as their email and profile picture. You can use these details to personalize the user interface of your app.
...
Verify that you can access the `email`, `picture`, or any other [claim](/docs/ja-jp/secure/tokens/id-tokens/id-token-structure) after you have logged in.
...
[Edit on GitHub](https://github.com/auth0/docs/edit/master/ja-jp/articles/quickstart/native/ios-swift/01-login.md)