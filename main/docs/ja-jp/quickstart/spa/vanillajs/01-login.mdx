...
'og:title': "Login"
'og:description': "This tutorial demonstrates how to add user login to a Javascript application using Auth0."
'og:image': "https://cdn2.auth0.com/docs/1.14567.0/media/platforms/html5.png"
'twitter:title': "Login"
'twitter:description': "This tutorial demonstrates how to add user login to a Javascript application using Auth0."
---
...
5. [Initialize the SDK](#initialize-the-sdk)
6. [Evaluate the authentication state](#evaluate-the-authentication-state)
7. [Log In to the Application](#log-in-to-the-application)
8. [Log the User Out](#log-the-user-out)
9. [Read the User Profile](#read-the-user-profile)
...
Log In & Download Sample[View on Github](https://github.com/auth0-samples/auth0-javascript-samples/tree/master/01-Login)
...
[Universal Login](/docs/ja-jp/hosted-pages/login) is the easiest way to set up authentication in your application. We recommend using it for the best experience, best security and the fullest array of features. This guide will use it to provide a way for your users to log in to your JavaScript application.

<Callout icon="file-lines" color="#0EA5E9" iconType="regular">

You can also embed the login dialog directly in your application using the [Lock widget](/docs/ja-jp/lock). If you use this method, some features, such as single sign-on, will not be accessible.

</Callout>

When a user logs in, Auth0 returns three items:

* `access_token`: to learn more, see the [Access Token documentation](/docs/ja-jp/tokens/concepts/access-tokens)
* `id_token`: to learn more, see the [ID Token documentation](/docs/ja-jp/tokens/concepts/id-tokens)
...
If you are using the [classic Universal Login experience](/docs/ja-jp/universal-login/classic) and would like users to authenticate using **[social identity providers](/docs/ja-jp/connections#social)** (such as Google, Apple, Facebook, etc.), then you will need to configure those connections in your [Auth0 Dashboard](https://manage.auth0.com/dashboard).
...
If you are using the [new Universal Login experience](/docs/ja-jp/universal-login/new), the default enabled social connections will silently reauthenticate without additional configuration. However, you should still set up your own keys and avoid using [default Auth0 development keys](/docs/ja-jp/connections/social/devkeys#limitations-of-developer-keys) in a production app.
...
Authentication is achieved through a redirect to the Auth0 [Universal Login Page](/docs/ja-jp/hosted-pages/login). Once the user signs up or logs in, the result will be passed to your app's redirect URI, which is provided with the authorization request.
...
[* Part 2: Calling an API using an access token](/docs/ja-jp/quickstart/spa/vanillajs/02-calling-an-api)
...
[Edit on GitHub](https://github.com/auth0/docs/edit/master/ja-jp/articles/quickstart/spa/vanillajs/01-login.md)