...
Most single-page apps use resources from data APIs. You may want to restrict access to those resources, so that only authenticated users with sufficient privileges can access them. Auth0 lets you manage access to these resources using [API Authorization](/docs/api-auth).

This tutorial shows you how to access protected resources in your API.

<Note>

This tutorial does not show you how to add protection to your API. Read the [Backend/API quickstart documentation](/docs/quickstart/backend) for instructions on how to protect your API.
...
The values for `domain` and `clientId` should have already been specified as part of the [Login tutorial](/docs/quickstarts/spa/vanillajs/01-login). They should point to the Domain and Client ID values for your Auth0 app respectively.
