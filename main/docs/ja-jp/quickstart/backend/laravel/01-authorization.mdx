...
'og:title': "Add Authorization to a Laravel Application"
'og:description': "Auth0's Laravel SDK allows you to quickly add token-based authorization and route access control to your Laravel application. This guide demonstrates how to integrate Auth0 with a new or existing Laravel 9 or 10 application."
'og:image': "https://cdn2.auth0.com/docs/1.14567.0/media/platforms/php.png"
'twitter:title': "Add Authorization to a Laravel Application"
'twitter:description': "Auth0's Laravel SDK allows you to quickly add token-based authorization and route access control to your Laravel application. This guide demonstrates how to integrate Auth0 with a new or existing Laravel 9 or 10 application."
---

# Laravel API

<Frame>![Gravatar for evan.sims@auth0.com](//www.gravatar.com/avatar/f18513f6431d78d0891c146bfd59fbe6?d=mm&r=g&s=50)</Frame>
...
6. [Retrieve User Information](#retrieve-user-information)
7. [Run the Application](#run-the-application)
8. [Retrieve a Test Token](#retrieve-a-test-token)
...
Log In & Download Sample[View on Github](https://github.com/auth0-samples/laravel/tree/7.x/sample)

System requirements: Laravel 9 / 10 | PHP 8.0+ | Composer

---

**Backend applications differ from traditional web applications in that they do not handle user authentication or have a user interface. They provide an API that other applications can interact with. They accept [access tokens](/docs/ja-jp/secure/tokens/access-tokens) from `Authorization` headers in requests to control access to routes.**

Separate front-end applications are usually built to interact with these types of backends. These can be anything from [single-page applications](/docs/ja-jp/quickstart/spa) or [native or mobile apps](/docs/ja-jp/quickstart/native) (all of which Auth0 also provides SDKs for!)
...
As their name implies, [access tokens](/docs/ja-jp/secure/tokens/access-tokens) are designed to address matters of access control (authorization), and do not contain information about the user. **Backend applications work exclusively with access tokens.** You can retrieve information about the user who created the token using the [Management API](https://auth0.com/docs/api/management/v2), which we will demonstrate later.
...
You can also require the provided token to have specific [permissions](/docs/ja-jp/manage-users/access-control/rbac) by combining this with Laravel's `can` middleware:
...
You can learn more about [retrieving access tokens here](/docs/ja-jp/secure/tokens/access-tokens/get-access-tokens). For this quickstart, however, you can simply use an access token from [your API settings' "test" view](https://manage.auth0.com/#/apis).
...
[Edit on GitHub](https://github.com/auth0/docs/edit/master/ja-jp/articles/quickstart/backend/laravel/01-authorization.md)