...
'og:title': "Login"
'og:description': "This tutorial demonstrates how to add user login to a Java Servlet application."
'og:image': "https://cdn2.auth0.com/docs/1.14567.0/media/platforms/java.png"
'twitter:title': "Login"
'twitter:description': "This tutorial demonstrates how to add user login to a Java Servlet application."
---
...
3. [Create the AuthenticationController](#create-the-authenticationcontroller)
4. [Trigger Authentication](#trigger-authentication)
5. [Display the Home Page](#display-the-home-page)
...
Log In & Download Sample[View on Github](https://github.com/auth0-samples/auth0-servlet-sample/tree/master/01-Login)
...
* **auth0-java-mvc-commons**: is the [Java library](https://github.com/auth0/auth0-java-mvc-common) that allows you to use Auth0 with Java for server-side MVC web apps. It generates the Authorize URL that you need to call in order to authenticate and validates the result received on the way back to finally obtain the [Auth0 Tokens](/docs/ja-jp/tokens) that identify the user.
...
To enable users to login, your application will redirect them to the [Universal Login](/docs/ja-jp/universal-login) page. Using the `AuthenticationController` instance, you can generate the redirect URL by calling the `buildAuthorizeUrl(HttpServletRequest request, HttpServletResponse response, String redirectUrl)` method. The redirect URL must be the URL that was added to the **Allowed Callback URLs** of your Auth0 Application.
...
<Frame>![Display Token](https://cdn2.auth0.com/docs/1.14567.0/media/articles/java/display-token.png)</Frame>
...
[Edit on GitHub](https://github.com/auth0/docs/edit/master/ja-jp/articles/quickstart/webapp/java/01-login.md)