...
**New to Auth?** Learn [How Auth0 works](/docs/ja-jp/overview), how it [integrates with Regular Web Applications](/docs/ja-jp/architecture-scenarios/web-app-sso) and which [protocol](/docs/ja-jp/flows) it uses.
...
Now that users can log into your application, they need [a way to log out](/docs/ja-jp/logout/guides/logout-auth0). By default, when logout is enabled, Spring Security will log the user out of your application and clear the session. To enable successful logout of Auth0, you can provide a `LogoutHandler` to redirect users to your [Auth0 logout endpoint](https://auth0.com/docs/api/authentication?javascript#logout) (`https://{yourDomain}/v2/logout`) and then immediately redirect them to your application.
...
Verify that you can display the user name or [any other `user` property](/docs/ja-jp/users/references/user-profile-structure#user-profile-attributes) after you have logged in.
...
[Edit on GitHub](https://github.com/auth0/docs/edit/master/ja-jp/articles/quickstart/webapp/java-spring-boot/01-login.md)