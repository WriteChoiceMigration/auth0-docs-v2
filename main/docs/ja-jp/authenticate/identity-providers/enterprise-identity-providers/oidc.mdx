...
3. <Frame>![OIDC接続の詳細を入力する](/images/ja-jp/cdy7uua7fh8z/4PO4eBhEM3R0ZMhaTlDVfB/4811760e0343a8129485a2282eeae1ea/2025-02-25_09-45-59.png)</Frame>
   ...
   <tr>
   <td><strong>Communication Channel（通信チャネル）</strong></td>
   <td>**［Front Channel（フロントチャネル）］<strong>または</strong>［Back Channel（バックチャネル）］**に設定します。フロントチャネルはOIDCプロトコルを`response_mode=form_post`と`response_type=id_token`のパラメーターで使用します。バックチャネルは`response_type=code`を使用します。</td>
   </tr>
   <tr>
   <td><strong>Client ID（クライアントID）</strong></td>
   ...
   <td>接続プロファイルの変更方法について理解するには、「<a href="/docs/ja-jp/authenticate/identity-providers/enterprise-identity-providers/configure-pkce-claim-mapping-for-oidc">OIDC接続のPKCEとクレームのマッピングを構成する</a>」をお読みください。</td>
   </tr>
   </tbody>
   </table>
4. **［Provisioning（プロビジョニング）］**ビューでユーザープロファイルがAuth0で作成および更新される方法を構成することができます。
   ...
   <td>有効な場合、SCIMを使ってユーザープロファイルデータを同期することができます。詳細については、「<a href="/docs/ja-jp/authenticate/protocols/scim/configure-inbound-scim">インバウンドSCIMを構成する</a>」を参照してください。</td>
   </tr>
   </tbody>
   </table>
5. **［Login Experience（ログインエクスペリエンス）］**ビューでこの接続でのユーザーログインの方法を構成することができます。
   ...
   <td>ユーザーのメールドメインを、指定されたIDプロバイダードメインと比較します。詳細については、[識別子優先認証の構成]を参照してください。（/docs/ja-jp/authenticate/login/auth0-universal-login/identifier-first）</td>
   </tr>
   <tr>
