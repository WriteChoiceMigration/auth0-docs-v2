...
* 新しいトークンの取得には、[`/oauth/token`](/docs/api/authentication#refresh-token)エンドポイントを使用しなければなりません。これは、[`/delegation`](/docs/api/authentication#delegation)エンドポイントが廃止されたためです。
