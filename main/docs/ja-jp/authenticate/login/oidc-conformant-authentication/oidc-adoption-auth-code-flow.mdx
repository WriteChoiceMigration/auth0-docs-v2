...
返されるアクセストークンは不透明で、[`/userinfo`](/docs/ja-jp/api/authentication#get-user-info)エンドポイントの呼び出しに対してのみ有効です。
...
返されるアクセストークンは、`audience`パラメーターおよび[`/userinfo`](/docs/ja-jp/api/authentication#get-user-info)エンドポイントで指定されたAPIの呼び出しに対してオプションで有効です（APIが署名アルゴリズムとして`RS256`を使用し、`openid`が`scope`パラメーターとして使用されている場合）。
