...
* 返されるアクセストークンは、[`/userinfo`](/docs/api/authentication#get-user-info)エンドポイントの呼び出しに対して有効です。
...
* 返されるアクセストークンは、[`/userinfo`](/docs/api/authentication#get-user-info)エンドポイント（`audience`パラメーターで指定したAPIが`RS256`を[署名アルゴリズム](/docs/ja-jp/get-started/applications/signing-algorithms)として使用していることを条件とする）と、`audience`パラメーターで指定したリソースサーバー（任意）の呼び出しに対してのみ有効です。
...
* IDトークンを検証したら、アプリケーションはリプレイ攻撃を軽減するために<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">nonce</Tooltip>を検証する必要があります。
