...
以下の手順を完了して、<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">SAML</Tooltip> IDプロバイダーとしてOneLoginを構成します。
...
<tr>
<td><strong>X509 Signing Certificate（X509署名証明書）</strong></td>
...
最後に、接続用の<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">IdP</Tooltip>を起点とするログインを有効にし、SAMLアサーションが使用された後にユーザーがリダイレクトされるアプリケーションを選ぶようにします。詳細については、「[IDプロバイダー起点のシングルサインオンを構成する](/docs/ja-jp/authenticate/protocols/saml/saml-sso-integrations/identity-provider-initiated-single-sign-on)」をお読みください。

## 接続マッピングを編集する

OneLoginとAuth0を変更することなくそのままで使用すると、OneLoginを使ってログインし、<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-2" tip="">Auth0 Dashboard</Tooltip>で作成されたユーザーには、持っておきたい一部のプロファイル情報がありません。
