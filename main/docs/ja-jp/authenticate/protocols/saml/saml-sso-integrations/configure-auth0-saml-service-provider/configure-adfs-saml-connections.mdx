...
MicrosoftのActive Directory Federation Services（ADFS）へのカスタム<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">SAML</Tooltip>接続を作成して、マッピングを構成する際の柔軟性を高めます。カスタム接続を作成するには、次の操作を行う必要があります。
...
**［Add <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">Relying Party</Tooltip> Trust（証明者利用者信頼の追加）］**ウィザードを閉じると、**［Edit Claim Issuance Policy（要求発行ポリシーの編集）］**ウィンドウが表示されます。
...
<td>カスタムドメインが設定されている場合、<code>https://&lt;YOUR CUSTOM DOMAIN&gt;/login/callback?connection=YOUR_CONNECTION_NAME</code></td>
</tr>
<tr>
<td></td>
<td>カスタムドメインを使用しない場合、<code>https://{yourDomain}/login/callback?connection=YOUR_CONNECTION_NAME</code></td>
</tr>
<tr>
<td>Entity ID（エンティティID）</td>
<td><code>urn:auth0:{yourTenant}:YOUR_CONNECTION_NAME</code></td>
</tr>
</tbody>
</table>
