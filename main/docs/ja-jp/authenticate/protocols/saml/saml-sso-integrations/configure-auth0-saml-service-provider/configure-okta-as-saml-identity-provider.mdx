...
<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">SAML</Tooltip>エンタープライズ接続を構成して、Auth0のSAML IDプロバイダー（<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">IdP</Tooltip>）としてOktaを構成できます。
...
<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-2" tip="">Auth0 Dashboard</Tooltip>でSAMLエンタープライズ接続を作成できます。
...
Oktaユーザーがいない場合は、ユーザーを作成するか、[**IdP起点SSOを構成**](/docs/authenticate/protocols/saml/saml-sso-integrations/identity-provider-initiated-single-sign-on)する必要があります。
