...
このスクリプトは[レガシー認証シナリオ](/docs/ja-jp/authenticate/database-connections/custom-db/overview-custom-db-connections)でのみ使用され、ユーザーをAuth0と外部のデータベースから同時に削除したい場合には必須です。
...
これは疑似JavaScriptを使った例で、どのようにすれば`deleteUser`関数を実装できるかがわかります。言語固有の例については、「[言語固有のスクリプトの例](/docs/ja-jp/authenticate/database-connections/custom-db/templates/delete#language-specific-script-examples)」をお読みください。
