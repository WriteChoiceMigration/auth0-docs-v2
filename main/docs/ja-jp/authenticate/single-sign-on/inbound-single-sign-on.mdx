...
サービスプロバイダー開始のシングルサインオン（<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" tip="">SSO</Tooltip>）を実装する場合、Auth0はSSOサービスプロバイダー（SP）になります。ユーザーがアプリケーションにログインすると：
...
* ソーシャルIDプロバイダー
* Generic OAuth2 <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">Authorization Server</Tooltip>をAuth0に追加する
* [Auth0 Marketplace](https://marketplace.auth0.com/)

### SAML

* Auth0は、Auth0を使って以下の<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-2" tip="">SAML</Tooltip> <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-3" tip="">IdP</Tooltip>を構成するための手順を提供しています。
...
* <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-4" tip="">OpenID</Tooltip> Connect（OIDC）は、IdP-initiated SSOをサポートしていません。
