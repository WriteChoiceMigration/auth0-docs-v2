...
<tbody>
<tr>
<td><strong>Sign On URL（サインオンURL）</strong></td>
<td>Auth0で作成した直後のGitHubアプリケーションの<code>client_id</code>は<code>https://{yourDomain}/samlp/{CLIENT_ID}</code> <br/> <code>{CLIENT_ID}</code>になります。</td>
</tr>
<tr>
...
<tr>
<td><strong>Verification Certificate（検証証明書）</strong></td>
<td><code>https://{yourDomain}/pem</code>からダウンロードします。ダウンロードしたファイルをテキストエディターで開き、中身をコピーしてからGitHubのテキスト領域に貼り付けます。</td>
</tr>
<tr>
<td><strong>User Attributes（ユーザー属性）</strong></td>
